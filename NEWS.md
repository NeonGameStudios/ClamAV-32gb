...
- Changed the `cl_fmap_set_hash()` ABI to accept a digest pointer and bumped
  the libclamav SONAME. Applications using the previous scalar third
  argument must be rebuilt against this release.
...
- libclamav: Added a class of scan callback functions that can be added with the
  ...
  ```c
    ...
    cl_error_t cl_fmap_set_hash(const cl_fmap_t *map, const char *hash_alg, const unsigned char *hash);

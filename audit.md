...
| MBR, APM, and GPT partition scanners | Source-fixed and focused-tested | Malformed or truncated partition headers and table entries now reapply the sticky incomplete invariant at scanner finalization; APM entry signatures, native-width declared-size arithmetic, and nested partition ranges are checked before dispatch, preventing malformed metadata from normalizing to clean. |

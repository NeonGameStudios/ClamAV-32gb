...
| PDF, XDP, HWPML, and MSXML embedded-content callers | Fail-visible bounded support | Whole-map materialization was removed from audited entry paths. Legacy deep parsers have explicit 64 MiB caps; PDF raw/decoded stream and extracted-object limit/write failures now preserve non-clean status and refuse partial output; HWPML Base64 is streamed with strict quartet and padding validation; the MSXML embedded-content caller now opts out of parse-error suppression and marks truncated XML non-cacheable. |

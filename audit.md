...
| UDF, AutoIt, NSIS, EGG, and ARJ | Hardened in this milestone | Result inversion, unchecked arithmetic, partial-decode success, unknown non-empty methods, and scanner-facing whole-input decoder reads were converted to checked, bounded, fail-visible paths; AutoIt EA05/EA06 encrypted compressed input now decrypts through 64 KiB windows, while decoded output and stored-member paths remain explicitly bounded by the legacy contiguous-allocation ceiling; NSIS members now stream through 64 KiB input windows with quota-accounted output, including the stateful solid path; scanner-facing EGG members now decode through 64 KiB fmap/output windows into a `MaxTemporarySize`-reserved descriptor, while the compatibility byte-buffer API retains its individual-allocation guard; ARJ SFX member-range normalization and extraction/rewind failures are fail-visible; UDF's mandatory descriptor-area and descriptor-fetch failures now mark the scan incomplete and return `CL_EPARSE`. |
| DMG | Source-fixed and focused-tested | Truncated or invalid `koly` trailers now mark the scan incomplete; the XML resource fork is consumed through bounded SAX input, decoded `<data>` values use quota-accounted spools, and one `mish` block remains bounded at 64 MiB. Fork-relative offsets, stripe geometry, exact output, decoder completion, time limits, temporary admission, allocation, output-write, temporary-file, cleanup, and multi-segment rejection failures are fail-visible. Each stripe list requires exactly one zero-length final `END` record. |
...
| F-07 whole-input Rust parsers | Bounded staged support; qualification open | LHA uses bounded `Read + Seek`; ALZ now parses through the same adapter and streams member output into quota-accounted spools; OneNote stages its root through quota-accounted fmap windows and parses a disk-backed mapping, while modern and legacy attachment callbacks borrow member bytes directly into quota-accounted spools. The artificial 256 MiB admission cap is removed. The third-party OneNote root parser remains slice-based, and a real clamd RSS test remains open. |

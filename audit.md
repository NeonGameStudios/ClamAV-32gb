...
| UDF, AutoIt, NSIS, EGG, and ARJ | Hardened in this milestone | Result inversion, unchecked arithmetic, partial-decode success, unknown non-empty methods, and whole-input decoder reads were converted to checked, bounded, fail-visible paths; NSIS members now stream through 64 KiB input windows with quota-accounted output, including the stateful solid path; ARJ SFX member-range normalization and extraction/rewind failures are fail-visible; UDF's mandatory descriptor-area and descriptor-fetch failures now mark the scan incomplete and return `CL_EPARSE`. |

...
| F-07 whole-input Rust parsers | Bounded staged support; qualification open | LHA uses bounded `Read + Seek`; ALZ/OneNote stage roots through quota-accounted fmap windows and parse disk-backed mappings, removing the artificial 256 MiB admission cap. Rust input, mapping, parser, CRC, member, and panic failures remain sticky non-clean; a real clamd RSS test remains open. |

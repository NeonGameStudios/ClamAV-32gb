...
The authoritative current-source production-linked GCC harness passes `tar`
8/8, `tar_map` 3/3, `tar_corpus` 1/1, and `tar_member` 8/8 after the direct
TAR member test was given a valid root recursion layer. CPIO passes `cpio`
1/1 across all four materialized formats, `cpio_crc` 4/4, `cpio_numeric`
4/4, and `cpio_map` 6/6. SIS passes `sis` 1/1, `sis_structure` 2/2,
...
`clam.exe.bz2`, `clam.tar.gz`, and `clam.exe_and_mail.tar.gz`; the standalone
`bzip2 -t` check accepts the BZip2 fixture. With those inputs present, the
unchanged current-source production-linked GCC harness passes `bz_core` 7/7
and `bz_map` 5/5. The core case includes both BZip2/GZip nested-marker corpus
checks, concatenated BZip2 stream coverage, decoder-initialization failure,
and the legacy GZip fallback staging regression.
...
limit rejected the input first. Both the public root path and extracted child
path now return `CL_ERESOURCE` before temporary reservation or fmap creation;
the public path still publishes the root size to any structured report. The
conditional narrow-size child regression, source guards, and current-source
scanner review pin the child rejection. The attempted Docker warning-enabled
scanner syntax check is still blocked by the snapshot's known mixed generated
headers (`FILEBUFF`, Rust evidence types, and related declarations), not by
this boundary. Full current-object execution, ingress parity, sanitizer,
production-CVD/service, materialized large-file, Sonic1, and final release
qualification remain open.
...
The legacy PDF dictionary helpers accept signed `int` lengths. Stream-object
extraction previously narrowed the pointer distance to that type without a
boundary check, and encryption-dictionary handling narrowed the object size and
used an aliasing cast when locating `/CF`. A confirmed dictionary larger than
the helper width could therefore wrap or be misinterpreted before parsing.
`cli_pdf_legacy_dict_length()` now validates the conversion; stream extraction
rejects invalid bounds or lengths as sticky incomplete/resource results, and
encryption setup rejects oversized legacy dictionaries before helper access.
...
production-CVD/service, Sonic1, and final PDF qualification remain open.
...
caching with the reason `TAR input map is unavailable`; a valid map with no
...
count as though it were a byte count before reading the following attribute
record. A confirmed node could therefore make the walker read past the
declared node boundary. The parser now validates the encoded UTF-16 byte span,
the fixed attribute record, and the declared attribute payload before access,
returning fail-visible `CL_EFORMAT` with sticky incomplete/non-cacheable state.
The current-source `hfsplus.c` compiles warning-clean with GCC
`-Wall -Wextra -Wformat-security`; the current-source production-linked
`hfs_map` TCase passes 10/10, including the new boundary regression, and
...
matcher, and missing-map entry points now retain their explicit incomplete
status. The NSIS object is warning-clean under GCC `-Wall -Wextra
-Wformat-security`; full NSIS/MSXML/OOXML corpus, sanitizer, certified Linux,
production-CVD/service, Sonic1, and release qualification remain open.
...
Coverage includes MIME initial-read, line and header-lookahead, timeout,
oversized-line, encoded-attachment, streaming MHTML, MBR coordinate/read/
limit boundaries, and exact partition-child matching. This is bounded parser
and partition evidence, not full qualification; complete MIME/MHTML and
partition-image corpora, sanitizer, certified Linux x86-64, materialized
...
The current-source production-linked GCC `xar` case passes 8/8, `xar_map` and
`xar_metadata` pass 1/1 each, and the isolated `xar_corpus` case passes 1/1.
The corpus contains a complete compressed TOC and an in-range uncompressed
member; the XAR root does not begin with MZP, and the exact
`Xar.Member.MZ.UNOFFICIAL` alert is reached after bounded member extraction
and nested scanning. This supersedes the earlier mixed-object XAR crash
observation; it is bounded XAR TOC/member-handoff evidence, not complete XAR
qualification. Sanitizer, certified Linux x86-64, materialized large-file,
production-CVD/service, Sonic1, and release evidence remain open.
...
serializer emits the child end element while intentionally excluding the
`subdoc` wrapper from the nested scan. This is bounded subdocument handoff
evidence, not complete XAR qualification; sanitizer, certified Linux x86-64,
materialized large-file, production-CVD/service, Sonic1, and release evidence
remain open.
...
traversal. This is bounded APM validation and partition-handoff evidence, not
complete partition corpus qualification; sanitizer, certified Linux x86-64,
materialized large-file, production-CVD/service, Sonic1, and release evidence
remain open.
...
the isolated `jpeg_corpus` case 1/1. The corpus is a valid APP13 Photoshop
resource containing a bounded thumbnail payload; the outer JPEG does not begin
with `MZP`, and the exact `JPEG.Member.MZ.UNOFFICIAL` alert is reached through
the 8BIM thumbnail nested scan. This is bounded JPEG resource-extraction
evidence, not complete JPEG/image corpus qualification; sanitizer, certified
Linux x86-64, materialized large-file, production-CVD/service, Sonic1, and
release evidence remain open.
...
1/1 against the materialized `clam.exe.rtf` fixture. The exact embedded `MZP`
marker is absent from the outer RTF bytes and is detected after RTF object
decoding and nested handoff. The existing `rtf_map` case passes 8/8. Full RTF
corpus, sanitizer, certified Linux x86-64, materialized large-file,
production-CVD/service, Sonic1, and release qualification remain open.
...
Full HTML corpus, full-C ABI, sanitizer, certified Linux x86-64,
materialized large-file, production-CVD/service, Sonic1, and release
qualification remain open.
...
The current-source production-linked GCC `tnef` case passes 13/13 across
...
handling, temporary limit, and the materialized `clam.tnef` corpus fixture.
That fixture has no outer `MZP` marker and reaches an exact nested `MZP`
matcher result from the extracted attachment. The separate `tnef_map` case
passes 1/1 for missing-map admission, and timeout retains the canonical
...
decoder output chunks, and a fail-closed panic boundary. The dedicated
current-source production-linked `rust_lha` TCase passes 2/2 for the direct
and public-API in-range initial fmap callback failures: both return `CL_EREAD`,
record the sticky incomplete reason, and mark the map non-cacheable. The
shared `rust_map` case also passes 1/1 across the Rust parser entry points.
...
## Fileblob cleanup-context audit — 2026-08-27

`fileblobDestructiveDestroy()` is used while handling allocation, quota, and materialization failures. It previously assumed that any attached cleanup context also had a non-null engine when deciding whether to remove a temporary file. A failed or direct internal call with a context lacking an engine could therefore crash while unwinding the original error. Cleanup now treats a missing engine as the default remove-temporary policy, and a focused regression covers failed temporary admission followed by destructive cleanup. The capability remains pending until production-linked fileblob/mail execution and the complete release qualification evidence are available.
...
the helper. The same current-source check caught a stale footer call that
invoked a `SigBytes`-only method on the signing-service `Vec<u8>` result; the
footer now writes the returned bytes directly. The cached Rust 1.97.1 Docker

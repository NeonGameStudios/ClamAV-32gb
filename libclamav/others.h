...
/*
 * PCRE2 requires one contiguous subject. On a 64-bit build with anonymous
 * fmap paging, the subject can remain virtually contiguous while physical
 * pages are loaded on demand, so the large-file ceiling is the appropriate
 * upper bound. Builds without that fmap capability retain the historical
 * bounded-allocation limit and fail visibly above 1 GiB.
 */
#if defined(ANONYMOUS_MAP) && (SIZE_MAX > 0xffffffffU)
#define CLI_MAX_PCRE_CONTIGUOUS_FILESIZE CLI_MAX_LARGE_FILESIZE
#else
#define CLI_MAX_PCRE_CONTIGUOUS_FILESIZE CLI_MAX_ALLOCATION
#endif
...
typedef struct cli_flagged_sample {
    ...
    uint64_t size; /* A size of zero means size is unavailable (why would this ever happen?) */
    uint32_t hits;

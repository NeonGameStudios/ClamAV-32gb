...
static cl_error_t jpeg_parse_error(cli_ctx *ctx, const char *reason)
{
     ...
     * evidence object yet. Keep malformed JPEG input fail-visible there;
...
cl_error_t cli_parsejpeg(cli_ctx *ctx)
{
    ...
    while (1) {
        ...
        if (len < 2) {
            ...
            status = jpeg_parse_error(ctx, "Heuristics.Broken.Media.JPEG.InvalidSegmentSize");
            goto done;
        }
        if ((size_t)len > map->len - offset) {
            cli_warnmsg("JPEG: Segment data out of file\n");
            status = jpeg_parse_error(ctx, "Heuristics.Broken.Media.JPEG.SegmentDataOutOfFile");
            goto done;
        }
        offset += len;

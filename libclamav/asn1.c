...
static cl_error_t asn1_parse_countersignature(fmap_t *map, const void **asn1data, unsigned int *size, crtmgr *cmgr, const uint8_t *message, const unsigned int message_size, int64_t not_before, int64_t not_after, cli_ctx *scan_ctx)
{
    ...
    do {
        ...
        if (0 != cl_update_hash(hash_ctx, "\x31", 1)) {
            cl_hash_destroy(hash_ctx);
            cli_mark_scan_incomplete(scan_ctx, "Authenticode countersignature digest could not be initialized");
            break;
        }
        {
            struct cli_mapped_region attrs_region;
            cl_error_t hash_status;

            attrs_region.offset = fmap_ptr2off(map, attrs + 1);
            attrs_region.size   = (size_t)attrs_size - 1;
...
static cl_error_t asn1_parse_mscat(struct cl_engine *engine, fmap_t *map, size_t offset, unsigned int size, crtmgr *cmgr, int embedded, const void **hashes, unsigned int *hashes_size, cli_ctx *ctx)
{
    ...
    do {
        ...
        *hashes_size = deep.size;
        ...
        if (0 != cl_update_hash(hash_ctx, "\x31", 1)) {
            cl_hash_destroy(hash_ctx);
            cli_mark_scan_incomplete(ctx, "Authenticode attribute digest could not be initialized");
            break;
        }
        {
            struct cli_mapped_region attrs_region;
...
cl_error_t cli_hash_mapped_regions(fmap_t *map, void *hash_ctx, const struct cli_mapped_region *regions, uint32_t nregions, cli_ctx *ctx)
{
    ...
    if (NULL == map || NULL == hash_ctx || (0 != nregions && NULL == regions)) {
        cli_mark_scan_incomplete(ctx, "Authenticode hash received invalid region state");
        return CL_EARG;
    }

    for (i = 0; i < nregions; i++) {
        /* The subtraction form validates both the end coordinate and the
         * offset + size addition without allowing a native-width wrap. */
        if (regions[i].offset > map->len || regions[i].size > map->len - regions[i].offset) {
            cli_dbgmsg("cli_hash_mapped_regions: region %u is outside the mapped file\n", i);
            cli_mark_scan_incomplete(ctx, "Authenticode hash region is outside the mapped file");
            return CL_EREAD;
        }
    }

    for (i = 0; i < nregions; i++) {
        size_t at        = regions[i].offset;
        size_t remaining = regions[i].size;

        while (remaining != 0) {
            const uint8_t *data;
            size_t chunk = MIN(remaining, (size_t)CLI_AUTHENTICODE_HASH_CHUNK_SIZE);
            cl_error_t status;
            ...
            data = fmap_need_off_once(map, at, chunk);
            if (NULL == data) {
                cli_dbgmsg("cli_hash_mapped_regions: could not read region %u at offset %zu\n", i, at);
                cli_mark_scan_incomplete(ctx, "Authenticode hash region could not be read completely");
                return CL_EREAD;
            }

            if (0 != cl_update_hash(hash_ctx, data, chunk)) {
                cli_dbgmsg("cli_hash_mapped_regions: digest update failed for region %u\n", i);
                cli_mark_scan_incomplete(ctx, "Authenticode digest could not be computed completely");
                return CL_EVERIFY;
            }
...
cl_error_t asn1_check_mscat(struct cl_engine *engine, fmap_t *map, size_t offset, unsigned int size, const struct cli_mapped_region *regions, uint32_t nregions, cli_ctx *ctx)
{
    ...
    // across the required regions without materializing a whole large file.
    ret = cli_hash_mapped_regions(map, hash_ctx, regions, nregions, ctx);
    if (CL_SUCCESS != ret) {
        cl_hash_destroy(hash_ctx);
        return ret;
    }
    ...
    if (NULL != ctx && ctx->scan_incomplete) {
        cli_dbgmsg("asn1_check_mscat: refusing to trust a layer after an incomplete scan path\n");
        return CL_EPARSE;
    }
    ...
    ret = cli_trust_this_layer(ctx, "authenticode digital signature verification");
    if (CL_SUCCESS != ret)
        return ret;

...
static bool ole2_read_block(ole2_header_t *hdr, void *buff, size_t size, int32_t blockno)
{
    ...
    /* CFB sector identifiers are 32-bit, but the containing fmap byte
     * coordinate is not.  Calculate in 64 bits and narrow only after
     * checking the native size_t range. */
...
static size_t get_stream_data_offset(ole2_header_t *hdr, const property_t *word_block, uint32_t sector)
{
    ...
    const uint64_t sector_size = offset;
    uint64_t fib_offset        = 0;

    if (word_block->size < MINISTREAM_CUTOFF_SIZE) {
        if (hdr->sbat_root_start < 0) {
            return SIZE_MAX;
        }
        fib_offset = offset + sector_size * (uint32_t)hdr->sbat_root_start;
        fib_offset += (uint64_t)word_block->start_block * ((uint64_t)1 << hdr->log2_small_block_size);
    } else {
        fib_offset = offset + sector_size * sector;
    }

    return fib_offset > SIZE_MAX ? SIZE_MAX : (size_t)fib_offset;
}
...
static int ole2_walk_property_tree(ole2_header_t *hdr, const char *dir, int32_t prop_index,
                                   ...
                                   cli_ctx *ctx, uint64_t *scansize, cl_error_t *deferred_limit,
                                   void *handler_ctx,
                                   encryption_status_t *pEncryptionStatus)
{
    ...
    while (!ole2_list_is_empty(&node_list)) {
        ...
        if ((curindex < 0) || (curindex > (int32_t)hdr->max_block_no)) {
            cli_dbgmsg("OLE2: property tree index is outside the input map\n");
            cli_mark_scan_incomplete(ctx, "OLE2 property tree index is outside the input map");
            ole2_list_delete(&node_list);
            return CL_EPARSE;
        }
        ...
        for (i = 0; i < idx; i++) {
            ...
            if (current_block < 0) {
                ...
                cli_mark_scan_incomplete(ctx, "OLE2 property sector chain could not be read");
                ole2_list_delete(&node_list);
                return CL_EREAD;
            }
        }
        ...
        if (!ole2_read_block(hdr, prop_block, 512, current_block)) {
            cli_dbgmsg("OLE2: property tree could not be read\n");
            cli_mark_scan_incomplete(ctx, "OLE2 property tree could not be read");
            ole2_list_delete(&node_list);
            return CL_EREAD;
        }
        if (prop_block[idx].type <= 0) {
            cli_dbgmsg("OLE2: property tree contains an invalid entry type\n");
            cli_mark_scan_incomplete(ctx, "OLE2 property tree contains an invalid entry type");
            ole2_list_delete(&node_list);
            return CL_EPARSE;
        }
        ...
        if (cli_bitset_test(hdr->bitset, (unsigned long)curindex)) {
            ...
            cli_mark_scan_incomplete(ctx, "OLE2 property tree contains a loop");
            ole2_list_delete(&node_list);
            return CL_EPARSE;
        }
        ole2_listmsg("setting bitset\n");
        if (!cli_bitset_set(hdr->bitset, (unsigned long)curindex)) {
            cli_dbgmsg("OLE2: unable to mark property tree entry as visited\n");
            cli_mark_scan_incomplete(ctx, "OLE2 property tree visit tracking failed");
            ole2_list_delete(&node_list);
            return CL_EMEM;
        }
        ...
        switch (prop_block[idx].type) {
            case 5: /* Root Entry */
                ...
                if ((curindex != 0) || (rec_level != 0) ||
                    ...
                    cli_mark_scan_incomplete(ctx, "OLE2 property tree contains an illegal Root Entry");
                    ole2_list_delete(&node_list);
                    return CL_EPARSE;
                }
                hdr->sbat_root_start = prop_block[idx].start_block;
                if ((int)(prop_block[idx].child) != -1) {
                    ret = ole2_walk_property_tree(hdr, dir, prop_block[idx].child, handler, rec_level + 1, file_count, ctx, scansize, deferred_limit, handler_ctx, pEncryptionStatus);
                    if (ret != CL_SUCCESS) {
                        ole2_list_delete(&node_list);
                        return ret;
                    }
            ...
            case 2: /* File */
                ...
                if (within_limits) {
                    ...
                    *scansize -= prop_block[idx].size;
                    ...
                    cli_dbgmsg("OLE2: embedded file exceeds configured scan limits\n");
                    if (deferred_limit && *deferred_limit == CL_SUCCESS)
                        *deferred_limit = CL_EMAXSIZE;
                    cli_append_potentially_unwanted_if_heur_exceedsmax(ctx, limit_name, CL_EMAXSIZE);
                }
                if ((int)(prop_block[idx].child) != -1) {
                    ret = ole2_walk_property_tree(hdr, dir, prop_block[idx].child, handler, rec_level, file_count, ctx, scansize, deferred_limit, handler_ctx, pEncryptionStatus);
                    if (ret != CL_SUCCESS) {
                        ole2_list_delete(&node_list);
                        return ret;
                    }
            ...
            case 1: /* Directory */
                ...
                if (dir) {
                    ...
                    dirname = NULL;
                if ((int)(prop_block[idx].child) != -1) {
                    ret = ole2_walk_property_tree(hdr, dirname, prop_block[idx].child, handler, rec_level + 1, file_count, ctx, scansize, deferred_limit, handler_ctx, pEncryptionStatus);
                    if (ret != CL_SUCCESS) {
                        ole2_list_delete(&node_list);
                        if (dirname) {
                            free(dirname);
            ...
            default:
                ...
                cli_mark_scan_incomplete(ctx, "OLE2 property tree contains an unknown entry type");
                ole2_list_delete(&node_list);
                return CL_EPARSE;
        }
...
static cl_error_t handler_writefile(ole2_header_t *hdr, property_t *prop, const char *dir, cli_ctx *ctx, void *handler_ctx)
{
    ...
    if (prop->name_size > 64) {
        ...
        cli_mark_scan_incomplete(ctx, "OLE2 VBA stream name could not be represented");
        ret = CL_EPARSE;
        goto done;
    }
    ...
    if (name) {
        ...
        if (CL_SUCCESS != (ret = uniq_add(hdr->U, name, strlen(name), &hash, &cnt))) {
            cli_dbgmsg("OLE2 [handler_writefile]: too many property names added to uniq store.\n");
            cli_mark_scan_incomplete(ctx, "OLE2 VBA stream name index could not be completed");
            goto done;
        }
    } else {
        if (CL_SUCCESS != (ret = uniq_add(hdr->U, NULL, 0, &hash, &cnt))) {
            cli_dbgmsg("OLE2 [handler_writefile]: too many property names added to uniq store.\n");
            cli_mark_scan_incomplete(ctx, "OLE2 VBA stream name index could not be completed");
            goto done;
        }
    ...
    if (ofd < 0) {
        ...
        cli_mark_scan_incomplete(ctx, "OLE2 VBA stream could not be materialized");
        ret = CL_ECREAT;
        goto done;
    }
    ...
    if (!blk_bitset) {
        ...
        cli_mark_scan_incomplete(ctx, "OLE2 VBA stream block map could not be initialized");
        ret = CL_EMEM;
        goto done;
    }
    ...
    while ((current_block >= 0) && (len > 0)) {
        ...
        if (cli_bitset_test(blk_bitset, (unsigned long)current_block)) {
            ...
            cli_mark_scan_incomplete(ctx, "OLE2 VBA stream block chain contains a loop");
            ret = CL_EPARSE;
            break;
        }

        if (!cli_bitset_set(blk_bitset, (unsigned long)current_block)) {
            cli_mark_scan_incomplete(ctx, "OLE2 VBA stream block map could not be updated");
            ret = CL_EMEM;
            break;
        }
    ...
    if (ret == CL_BREAK) {
        if (len == 0) {
            ...
            cli_mark_scan_incomplete(ctx, "OLE2 VBA stream ended before all bytes were materialized");
            ret = CL_EPARSE;
        }
    } else if (ret != CL_SUCCESS) {
        cli_mark_scan_incomplete(ctx, "OLE2 VBA stream materialization failed");
    }
...
static cl_error_t scan_for_xlm_macros_and_images(ole2_header_t *hdr, property_t *prop, cli_ctx *ctx, bool *found_macro, bool *found_image)
{
    ...
    if (status != CL_SUCCESS) {
        cli_mark_scan_incomplete(ctx, "OLE2 XLM/image stream could not be scanned completely");
    }
    return status;
}
...
static cl_error_t handler_enum(ole2_header_t *hdr, property_t *prop, const char *dir, cli_ctx *ctx, void *handler_ctx)
{
    ...
    if (!pEncryptionStatus->encrypted && (!hdr->has_xlm || !hdr->has_image)) {
        ...
        if (name && (strcmp(name, "workbook") == 0 || strcmp(name, "book") == 0)) {
            status = scan_for_xlm_macros_and_images(hdr, prop, ctx, &hdr->has_xlm, &hdr->has_image);
            if (status != CL_SUCCESS) {
                goto done;
            }
        }
    ...
    if (status != CL_SUCCESS) {
        cli_mark_scan_incomplete(ctx, "OLE2 property enumeration failed");
    }
...
static cl_error_t scan_mso_stream(int fd, const char *filepath, cli_ctx *ctx)
{
    ...
    /* Do not scan partial output: only a terminal zlib stream is complete. */
    if (zret != Z_STREAM_END) {
        cli_mark_scan_incomplete(ctx, "MSO stream did not reach a complete zlib terminal state");
        ret = CL_EPARSE;
        goto mso_end;
    }
...
static cl_error_t handler_otf(ole2_header_t *hdr, property_t *prop, const char *dir, cli_ctx *ctx, void *handler_ctx)
{
    ...
    if (len != 0) {
        cli_mark_scan_incomplete(ctx, "OLE2 stream could not be materialized completely");
        if (ret == CL_BREAK) {
            ret = CL_EPARSE;
        }
        goto done;
    ...
    if (ret != CL_SUCCESS && ret != CL_VIRUS) {
        cli_mark_scan_incomplete(ctx, "OLE2 embedded stream scan failed");
        goto done;
    }
...
static cl_error_t handler_otf_encrypted(ole2_header_t *hdr, property_t *prop, const char *dir, cli_ctx *ctx, void *handler_ctx)
{
    ...
    if (stream_bytes_read < prop->size) {
        cli_mark_scan_incomplete(ctx, "OLE2 encrypted stream could not be materialized completely");
        if (ret == CL_BREAK) {
            ret = CL_EPARSE;
        }
        goto done;
    ...
    if (ret != CL_SUCCESS && ret != CL_VIRUS) {
        cli_mark_scan_incomplete(ctx, "OLE2 embedded encrypted stream scan failed");
        goto done;
    }
    ret = ret == CL_VIRUS ? CL_VIRUS : CL_SUCCESS;

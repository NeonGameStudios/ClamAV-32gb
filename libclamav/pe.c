...
static cl_error_t cli_hashsect(cli_ctx *ctx, const struct cli_exe_section *s, uint8_t **digest, const bool *generate)
{
    ...
    if (!ctx || !ctx->fmap || !s || !digest || !generate)
        return CL_ENULLARG;

    if (!s->rsz)
        return CL_BREAK;

    region.offset = (size_t)s->raw;
    region.size   = (size_t)s->rsz;
    ...
    for (type = CLI_HASH_MD5; type < CLI_HASH_AVAIL_TYPES; type++) {
        ...
        hash_ctx = cl_hash_init(cli_hash_name(type));
        if (NULL == hash_ctx)
            return CL_EMEM;

        status = cli_hash_mapped_regions(ctx->fmap, hash_ctx, &region, 1, ctx);
...
static cl_error_t scan_pe_mdb(cli_ctx *ctx, struct cli_exe_section *exe_section)
{
    ...
    for (type = CLI_HASH_MD5; type < CLI_HASH_AVAIL_TYPES; type++) {
        ...
        generate[type]  = foundsize[type] || foundwild[type] ||
                         (CLI_HASH_MD5 == type && cli_debug_flag && cli_always_gen_section_hash);
        if (generate[type]) {
    ...
    /* Generate hashes */
    ret = cli_hashsect(ctx, exe_section, hashset, generate);
    if (CL_SUCCESS != ret)
        goto end;
...
int cli_scanpe(cli_ctx *ctx)
{
    ...
    switch (peheader_ret) {
        case CL_EFORMAT:
            /* A malformed PE header means the PE-specific inspection path
             * could not be completed. Do not turn that skipped analysis into
             * a clean result or a clean-cache entry. Preserve the historical
             * broken-PE heuristic when it is configured to be terminal. */
            cli_mark_scan_incomplete(ctx, "PE header parsing ended with a malformed or truncated header");
            ret = CL_EFORMAT;
            if (DETECT_BROKEN_PE) {
        ...
        case CL_ERROR:
            cli_mark_scan_incomplete(ctx, "PE header parsing failed before PE-specific inspection completed");
            ret = CL_ERROR;
    ...
    if (peinfo->overlay_start <= UINT32_MAX && peinfo->overlay_size <= INT32_MAX) {
        pedata.overlays    = (uint32_t)peinfo->overlay_start;
        pedata.overlays_sz = (int32_t)peinfo->overlay_size;
    } else {
        pedata.overlays    = 0;
        pedata.overlays_sz = 0;
        cli_mark_scan_incomplete(ctx, "PE bytecode overlay metadata requires 32-bit coordinates");
    }
...
pe_legacy_unpackers_done:
    ...
    if (!bc_ctx) {
        cli_errmsg("cli_scanpe: can't allocate memory for bc_ctx\n");
        cli_mark_scan_incomplete(ctx, "PE bytecode unpacker context could not be allocated"); cli_exe_info_destroy(peinfo);
        return CL_EMEM;
    }
...
#define add_chunk_to_hash_list(_offset, _size) \
    ...
    /* MZ to checksum */
    at   = 0;
    hlen = (size_t)peinfo->e_lfanew + sizeof(struct pe_image_file_hdr) + (peinfo->is_pe32plus ? offsetof(struct pe_image_optional_hdr64, CheckSum) : offsetof(struct pe_image_optional_hdr32, CheckSum));
    if (hlen > fsize) {
        cli_dbgmsg("cli_check_auth_header: checksum field is outside the mapped file\n");
        goto finish;
    }
    add_chunk_to_hash_list(0, hlen);
    if (hlen > SIZE_MAX - 4) {
        cli_mark_scan_incomplete(ctx, "Authenticode header coordinate overflowed");
        ret = CL_EPARSE;
        goto finish;
    }
    at = hlen + 4;
    ...
    if (at > fsize || hlen > fsize - at) {
        cli_dbgmsg("cli_check_auth_header: optional header is outside the mapped file\n");
        goto finish;
    }
    add_chunk_to_hash_list(at, hlen);
    if (hlen > SIZE_MAX - 8 || at > SIZE_MAX - (hlen + 8)) {
        cli_mark_scan_incomplete(ctx, "Authenticode header coordinate overflowed");
        ret = CL_EPARSE;
        goto finish;
    }
    ...
    if (sec_dir_offset) {
        ...
        if ((size_t)sec_dir_offset > fsize ||
            (size_t)sec_dir_size > fsize - (size_t)sec_dir_offset ||
            fsize != (size_t)sec_dir_offset + (size_t)sec_dir_size) {
        ...
        at = (size_t)sec_dir_offset + sizeof(cert_hdr);
        hlen -= sizeof(cert_hdr);

        ret = asn1_check_mscat((struct cl_engine *)(ctx->engine), map, at, (unsigned int)hlen, regions, nregions, ctx);
    ...
    for (i = 0; i < (sizeof(supported_hashes) / sizeof(supported_hashes[0])); i++) {
        ...
        ret = cli_hash_mapped_regions(map, hashctx, regions, nregions, ctx);
        if (CL_SUCCESS != ret) {
            goto finish;
        }
        ...
        if (cli_hm_scan(authsha, 2, NULL, ctx->engine->hm_fp, hashtype) == CL_VIRUS) {
            ...
            if (ctx->scan_incomplete) {
                cli_dbgmsg("cli_check_auth_header: refusing catalog trust after an incomplete scan path\n");
                ret = CL_EPARSE;
                goto finish;
            }
            ...
            ret = cli_trust_this_layer(ctx, source);
            if (CL_SUCCESS != ret)
                goto finish;
...
cl_error_t cli_genhash_pe(cli_ctx *ctx, unsigned int class, cli_hash_type_t type, stats_section_t *hashes)
{
    ...
    if (class == CL_GENHASH_PE_CLASS_SECTION) {
        ...
        for (i = 0; i < peinfo->nsections; i++) {
            /* Generate hashes */
            cl_error_t hash_status = cli_hashsect(ctx, &peinfo->sections[i], hashset, genhash);
            ...
            } else if (CL_BREAK != hash_status && peinfo->sections[i].rsz) {

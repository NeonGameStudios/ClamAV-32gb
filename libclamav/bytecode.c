...
{
    struct cli_bc_ctx *ctx = cli_max_calloc(1, sizeof(*ctx));
    if (!ctx) {
    ...
    for (i = (BC_START_TID - 65); i < bc->num_types - 1; i++) {
        ...
        switch (t) {
            ...
            case 5:
                ...
                ty->containedTypes = cli_max_malloc(sizeof(*ty->containedTypes));
                if (!ty->containedTypes) {
...
static cl_error_t add_selfcheck(struct cli_all_bc *bcs)
{
    ...
    bc->trusted     = 1;
    bc->num_globals = 1;
    bc->globals     = cli_max_calloc(1, sizeof(*bc->globals));
    if (!bc->globals) {
        cli_errmsg("Failed to allocate memory for globals\n");
        return CL_EMEM;
    }
    bc->globals[0] = cli_max_calloc(1, sizeof(*bc->globals[0]));
    if (!bc->globals[0]) {
        cli_errmsg("Failed to allocate memory for globals\n");
        return CL_EMEM;
    }
    bc->globaltys = cli_max_calloc(1, sizeof(*bc->globaltys));
    if (!bc->globaltys) {
    ...
    *bc->globals[0]  = 0;
    ...
    bc->num_types    = 5;
    bc->num_func     = 1;
    bc->funcs        = cli_max_calloc(1, sizeof(*bc->funcs));
    if (!bc->funcs) {
    ...
    func->numBB        = 1;
    func->returnType   = 32;
    func->types        = cli_max_calloc(1, sizeof(*func->types));
    if (!func->types) {
        cli_errmsg("Failed to allocate memory for types\n");
        return CL_EMEM;
    }
    func->types[0] = 32;
    func->BB       = cli_max_calloc(1, sizeof(*func->BB));
    if (!func->BB) {
        cli_errmsg("Failed to allocate memory for BB\n");
        return CL_EMEM;
    }
    func->allinsts = cli_max_calloc(2, sizeof(*func->allinsts));
    if (!func->allinsts) {
    ...
    func->constants    = cli_max_calloc(1, sizeof(*func->constants));
    if (!func->constants) {
    ...
    inst->u.ops.numOps  = 1;
    inst->u.ops.opsizes = NULL;
    inst->u.ops.ops     = cli_max_calloc(1, sizeof(*inst->u.ops.ops));
    if (!inst->u.ops.ops) {
                                ...
                                unsigned id, fmap_t *map)
{
    ...
    if (!cctx || !engine || !ctx || !map)
        return CL_ENULLARG;
    if (id < _BC_START_HOOKS || id >= _BC_LAST_HOOK)
        return CL_EARG;

    hooks     = engine->hooks[id - _BC_START_HOOKS];
    hooks_cnt = engine->hooks_cnt[id - _BC_START_HOOKS];
    ...
    for (i = 0; i < hooks_cnt; i++) {
        ...
        if (bc->lsig &&
            (!bc->hook_lsig_id || !cctx->hook_lsig_matches ||
    ...
    if (!applicable) {
        cli_dbgmsg("Bytecode: no applicable bytecode registered for hook %u\n", id);
        return CL_SUCCESS;
    }

...
struct cli_subsig_matches {
    uint32_t last;
    uint32_t next;
    uint64_t offsets[16]; /* offsets[] is variable length */
};
...
typedef struct cli_ac_data {
    ...
    uint32_t **lsigcnt;
    uint64_t **lsigsuboff_last, **lsigsuboff_first;
    struct cli_lsig_matches **lsig_matches;
    uint8_t *yr_matches;
    uint64_t *offset;
    uint64_t macro_lastmatch[32];
...
/* Keep the checked-in Rust representation honest on the supported 64-bit
 * targets. The Rust crate repeats these checks with offset_of!/size_of!. */
typedef char cli_ac_patt_abi_size[(sizeof(struct cli_ac_patt) == 160) ? 1 : -1];
typedef char cli_ac_patt_abi_offdata[(offsetof(struct cli_ac_patt, offdata) == 104) ? 1 : -1];

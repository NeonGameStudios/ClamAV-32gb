                                ...
                                size_t *sum, uint64_t *temporary_reserved)
{
    ...
    if (UINT64_MAX - (uint64_t)*sum < (uint64_t)len)
        needed = UINT64_MAX;
    else
...
cl_error_t cli_pdf(const char *dir, cli_ctx *ctx, off_t offset)
{
    ...
    if (q <= eofmap) {
        ...
        if (q <= eofmap) {
            ...
            if (CL_SUCCESS != cli_strntol_wrap(q, q - eofmap + map_off, 0, 10, &temp_long)) {
                ...
                pdf.flags |= 1 << BAD_PDF_TRAILER;
                parse_error_reason = "PDF trailer xref could not be parsed";
            } else if (temp_long < 0) {
                cli_dbgmsg("cli_pdf: Encountered invalid negative PDF trailer xref (%ld).\n", temp_long);
                pdf.flags |= 1 << BAD_PDF_TRAILER;
                parse_error_reason = "PDF trailer xref is negative";
            } else {
                xref = (unsigned long)temp_long;
                if ((uint64_t)xref > map->len - (size_t)offset) {
                    cli_dbgmsg("cli_pdf: did not find valid xref\n");
                    pdf.flags |= 1 << BAD_PDF_TRAILER;
                    parse_error_reason = "PDF trailer xref is outside the input map";
                } else {
                    bytesleft = map->len - (size_t)offset - xref;
                    if (bytesleft > 4096)
                        bytesleft = 4096;
    ...
    if (parse_error_reason != NULL) {
        cli_mark_scan_incomplete(ctx, parse_error_reason);
        if (CL_SUCCESS == rc)
            rc = CL_EPARSE;
    }

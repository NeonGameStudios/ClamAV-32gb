...
static void
sis_note_cleanup_failure(cli_ctx *ctx, cl_error_t *status, int failed,
...
cl_error_t cli_scansis(cli_ctx *ctx)
{
...

    {
        ...
        if (nread != SIZEOF_HEADER_UUIDS) {
            ...
            sis_note_cleanup_failure(ctx, &read_status, cli_rmdirs(tmpd) != 0, CL_EUNLINK,
                                     "SIS temporary directory could not be removed");
            free(tmpd);
            return read_status;
        }
    }
            ...
            case PKGfile: {
                ...
                if (ftype != FTnull) {
                    ...
                    for (j = 0; j < fcount; j++) {
                        ...
                        if (status != CL_SUCCESS) {
                            sis_note_cleanup_failure(ctx, &status, close(fd) != 0, CL_EWRITE,
...
static inline int sis9x_field_is_expected(struct SISTREAM *s, uint32_t actual,
                                           uint32_t expected)
{
    if (actual == expected)
        return 1;

    /* A successfully read field of the wrong type is not an end marker. The
     * enclosing field still declared bytes, so accepting it as a loop exit
     * would silently discard malformed content and allow a clean result. */
    s->incomplete = 1;
    if (s->failure == CL_CLEAN)
        s->failure = CL_EPARSE;
    return 0;
...
static cl_error_t real_scansis9x(cli_ctx *ctx, const char *tmpd)
{
    ...
    while (1) { /* 1DATA */
        ...
        while (1) { /* DATA::ARRAY */
            ...
            if (sis9x_checktimelimit(ctx, s) != CL_SUCCESS)
                break;
            if (getfield(s, &field) || !sis9x_field_is_expected(s, field, T_ARRAY) ||
                !sis9x_field_has_bytes(s, 4) || getd(s, &atype) ||
                !sis9x_field_is_expected(s, atype, T_DATAUNIT))
                break;
            s->fsize[s->level] -= 4;
            ...
            while (s->fsize[s->level - 1]) { /* FOREACH DATA::ARRAY::DATAUNITs */
                ...
                while (1) { /* DATA::ARRAY::DATAUNIT[x]::ARRAY */
                    ...
                    if (getfield(s, &field) || !sis9x_field_is_expected(s, field, T_ARRAY) ||
                        !sis9x_field_has_bytes(s, 4) || getd(s, &atype) ||
                        !sis9x_field_is_expected(s, atype, T_FILEDATA))
                        break;
                    s->fsize[s->level] -= 4;
                    ...
                    while (s->fsize[s->level - 1]) { /* FOREACH DATA::ARRAY::DATAUNIT[x]::ARRAY::FILEDATA */
                        ...
                        while (1) { /* DATA::ARRAY::DATAUNIT[x]::ARRAY::FILEDATA[x]::COMPRESSED */
                            if (sis9x_checktimelimit(ctx, s) != CL_SUCCESS)
                                break;
                            if (getfield(s, &field) || !sis9x_field_is_expected(s, field, T_COMPRESSED) ||
                                !sis9x_field_has_bytes(s, 12) || getd(s, &field) ||
                                getd(s, &usize) || getd(s, &usizeh) || usizeh)
                                break;
                            s->fsize[s->level] -= 12;
                            ...
                            if (ret != CL_SUCCESS) {
                                ...
                                if (s->failure == CL_CLEAN)
                                    s->failure = ret;
                                sis_note_cleanup_failure(ctx, &ret, close(fd) != 0, CL_EWRITE,

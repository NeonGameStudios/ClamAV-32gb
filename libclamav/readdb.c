...
static int lsig_parse_uint64(const char *text, uint64_t *value)
{
    char *end = NULL;
        ...
        switch (apt->type) {
            ...
            case CLI_TDB_RANGE:
                ...
                *pt2++ = 0;
                ...
                if (!strcmp(apt->name, "FileSize")) {
                    uint64_t min_size, max_size;

                    if (tdb->filesize_range || !lsig_parse_uint64(pt, &min_size) || !lsig_parse_uint64(pt2, &max_size) || min_size > max_size) {
                        cli_errmsg("lsigattribs: Invalid argument for %s\n", tokens[i]);
                        return -1;
                    }

                    tdb->filesize_range = (uint64_t *)MPOOL_REALLOC2(tdb->mempool, NULL, 2 * sizeof(uint64_t));
                    if (!tdb->filesize_range)
                        return -1;

...
static int cli_bm_uint64_cmp(const void *a, const void *b)
{
...
cl_error_t cli_bm_addpatt(struct cli_matcher *root, struct cli_bm_patt *pattern, const char *offset)
{
    ...
    if (pattern->offdata[0] != CLI_OFF_ANY64) {
...
cl_error_t cli_bm_initoff(const struct cli_matcher *root, struct cli_bm_off *data, const struct cli_target_info *info)
{
    ...
    data->cnt = data->pos = 0;
    data->offtab          = (uint64_t *)malloc(root->bm_patterns * sizeof(uint64_t));
    if (!data->offtab) {
        cli_errmsg("cli_bm_initoff: Can't allocate memory for data->offtab\n");
        return CL_EMEM;
    }
    data->offset = (uint64_t *)malloc(root->bm_patterns * sizeof(uint64_t));
    if (!data->offset) {
...
cl_error_t cli_bm_scanbuff(const unsigned char *buffer, uint32_t length, const char **virname, const struct cli_bm_patt **patt, const struct cli_matcher *root, uint64_t offset, const struct cli_target_info *info, struct cli_bm_off *offdata, cli_ctx *ctx)
{
    uint32_t i, j;
    uint64_t off, off_min, off_max, delta;
    ...
    if (offdata) {
        ...
        if (offdata->pos >= offdata->cnt)
            return CL_CLEAN;
        delta = offdata->offtab[offdata->pos] - offset;
        if (delta > (uint64_t)(UINT32_MAX - i))
            return CL_CLEAN;
        i += (uint32_t)delta;
    }
        ...
        if (shift == 0) {
            ...
            if (p && p->cnt == 1 && p->pattern0 != prefix) {
                if (offdata) {
                    ...
                    delta = offdata->offtab[offdata->pos] - off;
                    if (delta > (uint64_t)(UINT32_MAX - i))
                        return viruses_found > 0 ? CL_VIRUS : CL_CLEAN;
                    i += (uint32_t)delta;
                } else {
                    i++;
                }
            ...
            while (p) {
                ...
                if (offdata) {
                    ...
                    } else if ((offdata->offset[p->offset_min] == CLI_OFF_NONE64) || (offdata->offset[p->offset_min] != offset + off - p->prefix_length)) {
                        p = p->next;
                        continue;
                    }
                }

...
cl_error_t hm_addhash_bin(struct cl_engine *engine, hash_purpose_t purpose, const void *binhash, cli_hash_type_t type, uint64_t size, const char *virusname)
{
    ...
    if (size >= UINT32_MAX) {
        ...
        for (i = 0; i < root->hm.sizehashes64_count[type]; i++) {
            if (root->hm.sizehashes64[type][i].size == size) {
                szh64 = &root->hm.sizehashes64[type][i];
                break;
            }
        }
        ...
        if (!szh64) {
            ...
            if (!szh64) {
                ...
                cli_errmsg("hm_addhash_bin: failed to grow 64-bit size hash table\n");
                return CL_EMEM;
            }
            root->hm.sizehashes64[type]       = szh64;
            root->hm.sizehashes64_count[type] = count + 1;
            szh64                             = &szh64[count];
            memset(szh64, 0, sizeof(*szh64));
            szh64->size = size;
        }
        szh = &szh64->hashes;
    } else if (size) {
        ...
        item = cli_htu32_find(ht, (uint32_t)size);
        if (!item) {
...
static inline int hm_cmp(const uint8_t *itm, const uint8_t *ref, unsigned int keylen)
{
    ...
    /* Hash entries are byte-packed and are not guaranteed to be aligned. */
    memcpy(&i, itm, sizeof(i));
...
bool cli_hm_have_size(const struct cli_matcher *root, cli_hash_type_t type, uint64_t size)
{
    ...
    if (size >= UINT32_MAX) {
        for (i = 0; i < root->hm.sizehashes64_count[type]; i++) {
            if (root->hm.sizehashes64[type][i].size == size)
                return root->hm.sizehashes64[type][i].hashes.items != 0;
        }
        return false;
    }

    return root->hm.sizehashes[type].capacity && cli_htu32_find(&root->hm.sizehashes[type], (uint32_t)size);
}
...
{
    return (root && (root->hwild.hashes[type].items || root->hm.sizehashes[type].capacity || root->hm.sizehashes64_count[type]));
}
...
cl_error_t cli_hm_scan(const uint8_t *digest, uint64_t size, const char **virname, const struct cli_matcher *root, cli_hash_type_t type)
{
    ...
    if (size >= UINT32_MAX) {
        for (i = 0; i < root->hm.sizehashes64_count[type]; i++) {
            if (root->hm.sizehashes64[type][i].size == size)
                return hm_scan(digest, virname, &root->hm.sizehashes64[type][i].hashes, type);
        }
        return CL_CLEAN;
    }
    ...
    item = cli_htu32_find(&root->hm.sizehashes[type], (uint32_t)size);
    if (!item)
        return CL_CLEAN;
...
void hm_free(struct cli_matcher *root)
{
    ...
    for (type = CLI_HASH_MD5; type < CLI_HASH_AVAIL_TYPES; type++) {
        ...
        for (i = 0; i < root->hm.sizehashes64_count[type]; i++) {
            struct cli_sz_hash *szh = &root->hm.sizehashes64[type][i].hashes;
        ...
        MPOOL_FREE(root->mempool, root->hm.sizehashes64[type]);
        root->hm.sizehashes64_count[type] = 0;
    }

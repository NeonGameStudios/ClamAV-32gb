...
typedef char cli_lsig_tdb_abi_size[(sizeof(struct cli_lsig_tdb) == 144) ? 1 : -1];
typedef char cli_lsig_tdb_abi_filesize_range[(offsetof(struct cli_lsig_tdb, filesize_range) == 16) ? 1 : -1];
typedef char cli_lsig_tdb_abi_filesize[(offsetof(struct cli_lsig_tdb, filesize) == 80) ? 1 : -1];
...
/* The bytecode ABI and container metadata still use 32-bit sentinels.  Native
 * matcher offsets are uint64_t and must keep every 32-bit file position
 * representable, so use separate values outside the valid uint64_t offset
 * range for the widened paths. */
#define CLI_OFF_ANY         0xffffffffU
#define CLI_OFF_NONE        0xfffffffeU
#define CLI_OFF_ANY64       ((uint64_t)-1)
#define CLI_OFF_NONE64      ((uint64_t)-2)
...
/**
 * Enforce the platform-aware contiguous-subject policy used by PCRE matching.
 * On qualifying 64-bit fmap builds, PCREMaxFileSize values are constrained to
 * the 32-GiB large-file ceiling; other builds retain CLI_MAX_ALLOCATION.
 * Exceeding the effective limit marks a real scan incomplete and returns
 * CL_EMAXSIZE.
 */
...
/**
 * Convert native logical-signature offsets for the frozen 32-bit bytecode ABI.
 * Returns zero when the file size or any real match offset cannot be represented
 * without colliding with a legacy bytecode sentinel.
 */

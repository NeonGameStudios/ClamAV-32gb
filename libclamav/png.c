...
static cl_error_t png_parse_error(cli_ctx *ctx, const char *reason)
{
     ...
     * reporting behavior. */
    if (ctx == NULL || ctx->this_layer_evidence == NULL || !SCAN_HEURISTIC_BROKEN_MEDIA)
        return CL_EPARSE;
...
cl_error_t cli_parsepng(cli_ctx *ctx)
{
    ...
    while (1) {
        ...
        if (strcmp(chunk_type, "IHDR") == 0) {
            ...
            if (chunk_data_length != 0) {
                cli_dbgmsg("PNG: invalid IEND length: " STDu64 "\n", chunk_data_length);
                status      = png_parse_error(ctx, "Heuristics.Broken.Media.PNG.InvalidIENDLength");
                parse_error = true;
                goto scan_overlay;
            }
            have_IEND = true;
    ...
    if (!have_IEND) {
        ...
        status      = png_parse_error(ctx, "Heuristics.Broken.Media.PNG.MissingIEND");
        parse_error = true;
    }
...
scan_overlay:
    ...
    if (CL_SUCCESS == status) {
        ...
        if (map->len > offset) {
            ...
            if (ctx->engine == NULL) {
                cli_mark_scan_incomplete(ctx, "PNG overlay scan requires an owning engine");
                status = CL_ENULLARG;
                goto done;
            }

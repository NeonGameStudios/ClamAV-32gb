...
cl_error_t cli_pcre_check_size_limit(cli_ctx *ctx, uint64_t configured_limit, uint64_t needed)
{
    ...
    /* PCRE2 consumes one contiguous subject.  PCREMaxFileSize is a signature
     * execution policy, not permission to bypass libclamav's bounded
     * single-allocation policy.  In particular, the historical value of zero
     * must not turn a 32 GiB fmap into a 32 GiB residency request. */
    if (effective_limit == 0 || effective_limit > (uint64_t)CLI_MAX_ALLOCATION)
        effective_limit = (uint64_t)CLI_MAX_ALLOCATION;

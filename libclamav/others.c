...
static cl_error_t cli_validate_maxfilesize(uint64_t requested, uint64_t *validated)
{
    if (!validated)
        return CL_ENULLARG;

    if (requested == 0) {
        *validated = CLI_MAX_LARGE_FILESIZE;
        return CL_SUCCESS;
    }

    if (requested > CLI_MAX_LARGE_FILESIZE) {
        cli_errmsg("MaxFileSize: values above 32 GiB are not supported\n");
        return CL_EARG;
    }
...
static cl_error_t cli_validate_pcre_maxfilesize(uint64_t requested, uint64_t *validated)
{
    if (!validated)
        return CL_ENULLARG;

    if (requested == 0 || requested > (uint64_t)CLI_MAX_PCRE_CONTIGUOUS_FILESIZE) {
        *validated = (uint64_t)CLI_MAX_PCRE_CONTIGUOUS_FILESIZE;
        return CL_SUCCESS;
    }
    ...
    switch (field) {
        ...
        case CL_ENGINE_MAX_SCANTIME:
            if (num < 0 || (uint64_t)num > UINT32_MAX) {
                cli_errmsg("MaxScanTime: value does not fit the supported uint32 range\n");
                return CL_EARG;
            }
        ...
        case CL_ENGINE_PCRE_MAX_FILESIZE:
            if (num < 0) {
                cli_errmsg("PCREMaxFileSize: negative values are not allowed\n");
                return CL_EARG;
            }
            ret = cli_validate_pcre_maxfilesize((uint64_t)num, &engine->pcre_max_filesize);
            if (ret != CL_SUCCESS)
                return ret;
            break;
...
cl_error_t cl_engine_settings_apply(struct cl_engine *engine, const struct cl_settings *settings)
{
    ...
    engine->pcre_max_filesize   = pcre_max_filesize;

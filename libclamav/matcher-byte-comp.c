 ...
 *
 */
cl_error_t cli_bcomp_scanbuf(const unsigned char *buffer, size_t buffer_length, uint64_t buffer_offset, struct cli_ac_result **res, const struct cli_matcher *root, struct cli_ac_data *mdata, cli_ctx *ctx)
{
    size_t i;
    int val                = 0;
    ...
    for (i = 0; i < root->bcomp_metas; i++) {
        ...
        if (offset == CLI_OFF_NONE64) {
            offset = 0;
        }

        if (offset > INT64_MAX) {
            bcm_dbgmsg("cli_bcomp_scanbuf: skipping byte-compare offset above INT64_MAX: " STDu64 "\n", offset);
            continue;
        }

        /* The logical matcher records an absolute file coordinate, while
         * buffer is only the current matcher window. Convert the reference
         * coordinate to a signed window-relative value before adding the
         * byte-compare delta. A reference in the preceding window is valid
         * when its delta lands inside the current window. */
        if (offset >= buffer_offset) {
            distance = offset - buffer_offset;
            if (distance > INT64_MAX) {
                bcm_dbgmsg("cli_bcomp_scanbuf: reference offset is too far from current window\n");
                continue;
            }
            relative_offset = (int64_t)distance;
        } else {
            distance = buffer_offset - offset;
        ...
        bcomp_check = cli_bcomp_compare_check(buffer, buffer_length, relative_offset, bcomp);
        ...
        if (CL_VIRUS == bcomp_check) {
            /* check to see if we are being run in sigtool or not */
            if (bcomp->lsigid[0]) {
                /* Adjacent fmap matcher windows overlap. Compute the absolute
                 * comparison coordinate and use it to ensure that the same
                 * physical comparison is counted only once when it appears in
                 * both windows. Keep lsigsuboff_first untouched because the
                 * frozen bytecode ABI has historically exposed byte-compare
                 * subsignatures as having no direct match offset. */
                if (bcomp->offset < 0) {
                    distance = (uint64_t)(-(bcomp->offset + 1)) + 1;
                    if (distance > offset)
                        continue;
                    comparison_offset = offset - distance;
                } else {
                    distance = (uint64_t)bcomp->offset;
                    if (UINT64_MAX - offset < distance)
                        continue;
                    comparison_offset = offset + distance;
                }

                if (mdata->lsigsuboff_last[bcomp->lsigid[1]][bcomp->lsigid[2]] != CLI_OFF_NONE64 &&
                    comparison_offset <= mdata->lsigsuboff_last[bcomp->lsigid[1]][bcomp->lsigid[2]]) {
                    continue;
                }

                mdata->lsigcnt[bcomp->lsigid[1]][bcomp->lsigid[2]]++;
                mdata->lsigsuboff_last[bcomp->lsigid[1]][bcomp->lsigid[2]] = comparison_offset;
            } else {
...
cl_error_t cli_bcomp_compare_check(const unsigned char *f_buffer, size_t buffer_length, int64_t offset, struct cli_bcomp_meta *bm)
{
    ...
    /* Calculate the final buffer offset without signed overflow, then check
     * the unsigned buffer bounds. This keeps offsets above 2 GiB usable on
     * 64-bit hosts while rejecting negative or unrepresentable positions. */
    if ((bm->offset > 0 && offset > INT64_MAX - bm->offset) ||
        (bm->offset < 0 && offset < INT64_MIN - bm->offset)) {
        bcm_dbgmsg("cli_bcomp_compare_check: offset arithmetic overflow\n");
        goto done;
    }
    offset += bm->offset;

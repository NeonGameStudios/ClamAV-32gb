...
int cli_pespin_check_limits(cli_ctx *ctx, const struct cli_exe_section *sections, int sectcnt, uint32_t bitmap)
{
    uint64_t filesize = 0;
    int j;

    if (!ctx || !ctx->engine || !ctx->engine->maxfilesize)
        return 0;
...
int unspin(char *src, int ssize, struct cli_exe_section *sections, int sectcnt, uint32_t nep, int desc, cli_ctx *ctx)
{
    ...
    if (ctx->engine->maxfilesize) {
        if (cli_pespin_check_limits(ctx, sections, sectcnt, bitmap) != 0)
            return 2;
        bitmap = bitman;
    }

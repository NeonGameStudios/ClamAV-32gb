...
#define CLI_ZIP_INPUT_CHUNK_SIZE (64U * 1024U)

#ifdef UNZIP_STREAM_PRIVATE
struct zip_input {
    fmap_t *map;
    size_t offset;
    uint64_t remaining;
    uint8_t buffer[CLI_ZIP_INPUT_CHUNK_SIZE];
};

static inline cl_error_t zip_input_init(struct zip_input *input, fmap_t *map, size_t offset, uint64_t length)
{
    if (NULL == input || NULL == map)
        return CL_ENULLARG;

    if (offset > map->len || length > (uint64_t)(map->len - offset))
        return CL_EPARSE;
...
static inline cl_error_t zip_input_refill(struct zip_input *input, const uint8_t **data, unsigned int *length)
{
    size_t want;
    size_t got;

    if (NULL == input || NULL == data || NULL == length)
        return CL_ENULLARG;
    ...
    *length = 0;
    ...
    want = sizeof(input->buffer);
    if (input->remaining < want)
        want = (size_t)input->remaining;

    got = fmap_readn(input->map, input->buffer, input->offset, want);
    if (got != want)
        return CL_EREAD;

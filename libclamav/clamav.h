...
typedef char clamav_large_file_off_t_must_be_64_bit[(sizeof(off_t) == 8) ? 1 : -1];
#endif
 ...
 * Applications that need to inspect larger layers must use the scan-layer and
 * `cl_fmap_*` APIs exposed by `CL_SCAN_CALLBACK_PRE_SCAN`.
 */
 ...
 *                           Otherwise a CL_E* error code. In particular,
 *                           top-level MaxFileSize and MaxScanTime exhaustion
 *                           return CL_EMAXSIZE and CL_ETIMEOUT respectively

...
cl_error_t cli_pcre_recaloff(struct cli_matcher *root, struct cli_pcre_off *data, struct cli_target_info *info, cli_ctx *ctx)
{
    ...
    pm_dbgmsg("CLI_OFF_NONE64: " STDu64 "\n", CLI_OFF_NONE64);
    pm_dbgmsg("CLI_OFF_ANY64: " STDu64 "\n", CLI_OFF_ANY64);
    ...
    for (i = 0; i < root->pcre_metas; ++i) {
        ...
        if (pm->offdata[0] == CLI_OFF_ANY64) {
            data->offset[i] = CLI_OFF_ANY64;
            data->shift[i]  = 0;
        } else if (pm->offdata[0] == CLI_OFF_NONE64) {
            data->offset[i] = CLI_OFF_NONE64;
            data->shift[i]  = 0;
            ...
            if (data->offset[i] == CLI_OFF_ANY64) {
                data->offset[i] = CLI_OFF_ANY64;
                data->shift[i]  = 0;
            } else {
        ...
        pm_dbgmsg("%u: " STDu64 " " STDu64 "->" STDu64 "(+" STDu64 ")\n", i, pm->offdata[0], data->offset[i],
...
int cli_pcre_qoff(struct cli_pcre_meta *pm, size_t length, size_t *adjbuffer, size_t *adjshift)
{
    if (!pm)
        return CL_ENULLARG;

    /* default to scanning whole buffer but try to use existing offdata */
    if (pm->offdata[0] == CLI_OFF_NONE64) {
        return CL_BREAK;
    } else if (pm->offdata[0] == CLI_OFF_ANY64) {
        *adjbuffer = CLI_OFF_ANY64;
        *adjshift  = 0;
    } else if (pm->offdata[0] == CLI_OFF_ABSOLUTE) {
...
cl_error_t cli_pcre_scanbuf(const unsigned char *buffer, size_t length, const char **virname, struct cli_ac_result **res, const struct cli_matcher *root, struct cli_ac_data *mdata, const struct cli_pcre_off *data, cli_ctx *ctx)
{
    ...
    for (i = 0; i < root->pcre_metas; ++i) {
        ...
        if (data && data->offset[i] == CLI_OFF_NONE64) {
        ...
        if (!data) {
            ...
            /* Calculated offsets are file coordinates.  Do not silently
             * narrow them when this code is compiled for a 32-bit host. */
            if (data->offset[i] != (uint64_t)(size_t)data->offset[i] ||
                data->shift[i] != (uint64_t)(size_t)data->shift[i]) {
                pm_dbgmsg("cli_pcre_scanbuf: skipping offset not representable by size_t for /%s/\n", pd->expression);
                continue;
            }
        ...
        if (!rolling && !adjshift && (adjbuffer != CLI_OFF_ANY64))
            options |= PCRE2_ANCHORED;
        else
            options = 0;

        if (adjbuffer == CLI_OFF_ANY64)
            adjbuffer = 0;

        /* check the offset bounds */
        if (adjbuffer < length) {
            /* handle encompass flag */
            if (encompass && adjshift != 0 && adjshift != CLI_OFF_NONE64) {
            ...
            pm_dbgmsg("cli_pcre_scanbuf: starting offset is outside bounds of file %zu >= %zu\n", adjbuffer, length);
            continue;
        }
        ...
        do {
            ...
            if (rc > 0) {
                ...
                if (pm->lsigid[0]) {
                    size_t match_offset = adjbuffer + p_res.match[0];

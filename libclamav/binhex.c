    ...
    while (1) {
        ...
        if (!enc_todo || dec_done >= BH_FLUSH_SZ) {
            ...
            if (!enc_todo) {
                if (write_phase == IN_DATA) {
                    ...
                    cli_mark_scan_incomplete(ctx, "BinHex stream ended before its terminal state");
                    ret = CL_EPARSE;
                }
                break;
            }
        ...
        if (write_phase == IN_BANNER) {
            if ((char)b != ':') {
                ...
                cli_mark_scan_incomplete(ctx, "BinHex stream is missing its start identifier");
                ret = CL_EPARSE;
                break;
            }
            write_phase++;
        ...
        if (b > 0x7f || (b = hqxtbl[b]) == 0xff) {
            ...
            cli_mark_scan_incomplete(ctx, "BinHex stream contains an invalid encoded character");
            ret = CL_EPARSE;
            break;
        }

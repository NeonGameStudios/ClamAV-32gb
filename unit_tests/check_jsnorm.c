...
extern int __real_cl_update_hash(void *ctx, const void *data, size_t sz);
...
int clamav_test_fail_update_hash;
...
int __wrap_cl_update_hash(void *ctx, const void *data, size_t sz)
{
    if (clamav_test_fail_update_hash)
        return -1;
    return __real_cl_update_hash(ctx, data, sz);
}
...
static void tokenizer_test(const char *in, const char *expected, int split)
{
    ...
    cli_js_parse_done(state);
    ck_assert_int_eq(cli_js_output(state, tmpdir), CL_SUCCESS);
...
START_TEST(test_js_output_open_failure_is_visible)
{
    char missing_dir[1024];

    cli_js_process_buffer(state, "alert(1);", strlen("alert(1);"));
    cli_js_parse_done(state);
    snprintf(missing_dir, sizeof(missing_dir), "%s/missing-output-parent", tmpdir);
    ck_assert_int_ne(cli_js_output(state, missing_dir), CL_SUCCESS);
}
END_TEST
...
START_TEST(test_js_output_write_and_close_failures_are_visible)
{
    ...
    jsnorm_test_fail_write = 1;
    ck_assert_int_ne(cli_js_output(state, tmpdir), CL_SUCCESS);
    jsnorm_test_fail_write = 0;

    jsnorm_test_fail_close = 1;
    ck_assert_int_ne(cli_js_output(state, tmpdir), CL_SUCCESS);
    jsnorm_test_fail_close = 0;
}
END_TEST
#endif

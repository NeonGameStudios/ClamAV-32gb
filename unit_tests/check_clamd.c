...
START_TEST(test_maxscantime_parser_rejects_narrowing)
{
    const char *valid[]   = {"0", "4294967295"};
    const char *invalid[] = {"4294967296", "999999999999999999999999", "4294967295x", "+1", "-1", " 1", "1 "};
    size_t i;

    for (i = 0; i < sizeof(valid) / sizeof(valid[0]); i++) {
        struct optstruct *opts = optadditem("MaxScanTime", valid[i], 1, OPT_CLAMD, 0, NULL);
        const struct optstruct *option;
        ck_assert_ptr_nonnull(opts);
        option = optget(opts, "MaxScanTime");
        ck_assert_ptr_nonnull(option);
        ck_assert_int_eq(option->numarg, strtoll(valid[i], NULL, 10));
        optfree(opts);
    }

    for (i = 0; i < sizeof(invalid) / sizeof(invalid[0]); i++) {
        struct optstruct *opts = optadditem("MaxScanTime", invalid[i], 1, OPT_CLAMD, 0, NULL);
        ck_assert_ptr_null(opts);
    }

#ifndef _WIN32
    {
        char config_path[] = "/tmp/clamav-maxscantime-XXXXXX";
        int fd             = mkstemp(config_path);
        FILE *config;
        struct optstruct *opts;
        const struct optstruct *option;
        const char *invalid_config[] = {"4294967296", "999999999999999999999999", "4294967295x", "+1", "-1"};
        size_t invalid_index;
        ck_assert_int_ge(fd, 0);
        config = fdopen(fd, "w");
        ck_assert_ptr_nonnull(config);
        ck_assert_int_gt(fprintf(config, "MaxScanTime 0\n"), 0);
        ck_assert_int_eq(fclose(config), 0);
        opts = optparse(config_path, 0, NULL, 1, OPT_CLAMD, 0, NULL);
        ck_assert_ptr_nonnull(opts);
        option = optget(opts, "MaxScanTime");
        ck_assert_ptr_nonnull(option);
        ck_assert_int_eq(option->numarg, 0);
        optfree(opts);
        unlink(config_path);

        strcpy(config_path, "/tmp/clamav-maxscantime-XXXXXX");
        fd = mkstemp(config_path);
        ck_assert_int_ge(fd, 0);
        config = fdopen(fd, "w");
        ck_assert_ptr_nonnull(config);
        ck_assert_int_gt(fprintf(config, "MaxScanTime 4294967295\n"), 0);
        ck_assert_int_eq(fclose(config), 0);
        opts = optparse(config_path, 0, NULL, 1, OPT_CLAMD, 0, NULL);
        ck_assert_ptr_nonnull(opts);
        option = optget(opts, "MaxScanTime");
        ck_assert_ptr_nonnull(option);
        ck_assert_int_eq(option->numarg, 4294967295LL);
        optfree(opts);
        unlink(config_path);

        for (invalid_index = 0; invalid_index < sizeof(invalid_config) / sizeof(invalid_config[0]); invalid_index++) {
            strcpy(config_path, "/tmp/clamav-maxscantime-XXXXXX");
            fd = mkstemp(config_path);
            ck_assert_int_ge(fd, 0);
            config = fdopen(fd, "w");
            ck_assert_ptr_nonnull(config);
            ck_assert_int_gt(fprintf(config, "MaxScanTime %s\n", invalid_config[invalid_index]), 0);
            ck_assert_int_eq(fclose(config), 0);
...
START_TEST(test_scan_report_json_metadata_binds_completion_and_boundaries)
{
    static const char report[] =
        ...
        "\"logical_bytes\":34359738368,\"max_scan_size\":68719476736,"
    ...
    ck_assert_uint_eq(logical_bytes, UINT64_C(34359738368));
    ck_assert_uint_eq(max_scan_size, UINT64_C(68719476736));
...
START_TEST(test_largefile_admission_does_not_bypass_large_limits)
{
    struct cl_engine *engine = cl_engine_new();
    char reason[256];

    ck_assert_ptr_nonnull(engine);
    ck_assert_int_eq(cl_engine_set_num(engine, CL_ENGINE_MAX_FILESIZE, (long long)CLI_MAX_LARGE_FILESIZE), CL_SUCCESS);
...
START_TEST(test_maxscantime_cli_boundaries)
{
    ...
    opterr = 0;
    for (i = 0; i < sizeof(valid) / sizeof(valid[0]); i++) {
        char *argv[] = {"clamd", "--max-scantime", (char *)valid[i], NULL};
        struct optstruct *opts;
        const struct optstruct *option;

        optind = 1;
        opts   = optparse(NULL, 3, argv, 0, OPT_CLAMD, 0, NULL);
        ck_assert_ptr_nonnull(opts);
        option = optget(opts, "MaxScanTime");
        ck_assert_ptr_nonnull(option);
        ck_assert_int_eq(option->numarg, strtoll(valid[i], NULL, 10));
    ...
    for (i = 0; i < sizeof(invalid) / sizeof(invalid[0]); i++) {
        char *argv[] = {"clamd", "--max-scantime", (char *)invalid[i], NULL};
...
START_TEST(test_stream_client_rejects_over_limit)
{
    ...
    ck_assert_int_eq(socketpair(AF_UNIX, SOCK_STREAM, 0, sockets), 0);
    ck_assert_int_eq(send_stream_fd(sockets[0], fileno(regular), "regular-over-limit", &stream_limit), 0);
    close(sockets[0]);
    close(sockets[1]);
    fclose(regular);
    ...
    ck_assert_int_eq(fflush(regular), 0);
    ck_assert_int_eq(socketpair(AF_UNIX, SOCK_STREAM, 0, sockets), 0);
    ck_assert_int_eq(send_stream_fd(sockets[0], fileno(regular), "regular-at-limit", &stream_limit), 1);
    close(sockets[0]);
    close(sockets[1]);
    fclose(regular);
...
#define VCMDS_REPLY VERSION_REPLY "| COMMANDS: SCANREPORT CONTSCANREPORT MULTISCANREPORT ALLMATCHSCANREPORT FILDESREPORT INSTREAMREPORT SCAN QUIT RELOAD PING CONTSCAN VERSIONCOMMANDS VERSION END SHUTDOWN MULTISCAN FILDES STATS IDSESSION INSTREAM DETSTATSCLEAR DETSTATS ALLMATCHSCAN"
...
static Suite *test_clamd_suite(void)
{
    ...
    TCase *tc_parser, *tc_commands, *tc_stress;

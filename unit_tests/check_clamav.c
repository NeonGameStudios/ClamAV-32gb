...
START_TEST(test_html_normalized_view_uses_matcher_work_budget)
{
    static const unsigned char data[] =
     ...
     * total greater than the root size proves that normalized child bytes were
     * charged to the shared MaxMatcherWork budget. */
    engine->maxhtmlnormalize = 1024;
    engine->maxhtmlnotags   = 1024;
    memset(&options, 0, sizeof(options));
    options.parse = CL_SCAN_PARSE_HTML;
    ck_assert_int_eq(cl_engine_compile(engine), CL_SUCCESS);
    map = cl_fmap_open_memory(data, sizeof(data) - 1U);
...
START_TEST(test_rtf_implicit_object_close_status_is_fail_visible)
{
    ...
    ck_assert(ctx.scan_incomplete);
    ck_assert_str_eq(ctx.scan_incomplete_reason,
                     "RTF embedded object ended before its payload was complete");
    ck_assert(map->dont_cache_flag);
...
{
    UNUSEDPARAM(filename);
    UNUSEDPARAM(debug_flag);
    *hArchive    = &test_rar_handle;
    *comment     = NULL;
    *comment_size = 0;
    return UNRAR_OK;
}

static cl_unrar_error_t test_rar_peek(void *hArchive, unrar_metadata_t *file_metadata)
{
    UNUSEDPARAM(hArchive);
    memset(file_metadata, 0, sizeof(*file_metadata));
    strcpy(file_metadata->filename, "timeout-member");
    file_metadata->unpack_size = 1;
    return UNRAR_OK;
}

static cl_unrar_error_t test_rar_extract(void *hArchive, const char *destPath, char *outputBuffer,
                                         cl_unrar_progress_callback_t progress, void *progress_context)
{
    cli_ctx *ctx = (cli_ctx *)progress_context;
...
{
    UNUSEDPARAM(hArchive);
    UNUSEDPARAM(progress);
    UNUSEDPARAM(progress_context);
    return UNRAR_OK;
}

static void test_rar_close(void *hArchive)
{
    UNUSEDPARAM(hArchive);
}
...
START_TEST(test_rar_time_limit_callback_is_fail_visible)
{
    static const uint8_t data[] = {0};
...
START_TEST(test_sis_truncated_uid_header_is_parse_error)
{
    ...
    map = cl_fmap_open_memory(data, sizeof(data));
    ck_assert_ptr_nonnull(map);
    map->need = sis_truncated_uid_header_read_failure;
    ctx.engine            = &engine;
    ctx.options           = &options;
    ctx.fmap              = map;
    ctx.this_layer_tmpdir = tmpdir;
...
START_TEST(test_sis_time_limit_is_fail_visible)
{
    ...
    ck_assert(ctx.scan_timed_out);
    ck_assert(ctx.scan_incomplete);
    ck_assert_str_eq(ctx.scan_incomplete_reason, "SIS inspection reached the configured time limit");
    ck_assert(map->dont_cache_flag);
...
START_TEST(test_tar_end_marker_is_fail_visible)
{
    ...
    memcpy(data, "empty", sizeof("empty") - 1U);
    ...
    map = cl_fmap_open_memory(data, 512);
    ck_assert_ptr_nonnull(map);
    verdict    = CL_VERDICT_STRONG_INDICATOR;
    last_alert = "stale";
    scanned    = UINT64_MAX;
    ...
    map = cl_fmap_open_memory(data, 1024);
    ck_assert_ptr_nonnull(map);
    verdict    = CL_VERDICT_STRONG_INDICATOR;
    last_alert = "stale";
    scanned    = UINT64_MAX;
...
START_TEST(test_iso_missing_volume_descriptor_terminator_is_fail_visible)
{
    ...
    /* The primary descriptor is followed by a descriptor-shaped block but no
     * 0xff/CD001 terminator. The root directory is otherwise empty and valid,
     * so a descriptor walk that silently accepts EOF would return clean. */
    data[ISO_OFFSET] = 1;
    memcpy(data + ISO_OFFSET + 1, "CD001", 5);
    data[ISO_OFFSET + 128] = 0x00;
    data[ISO_OFFSET + 129] = 0x08; /* 2048-byte logical blocks */
...
START_TEST(test_iso_long_directory_name_is_fail_visible)
{
    enum {
        ISO_OFFSET = 32768,
        ROOT_BLOCK  = 32,
...
START_TEST(test_xar_toc_missing_root_close_is_fail_visible)
{
    static const uint8_t toc[] = "<?xml version=\"1.0\"?><xar><toc><file><data><offset>0</offset><length>1</length><size>1</size></data></file></toc>";
    uint8_t *data;
    size_t data_length;
    struct cl_scan_options options;
    struct cl_engine *scan_engine;
    ...
    data = xar_test_make_archive_from_toc(toc, sizeof(toc) - 1U, &data_length);
    ck_assert_ptr_nonnull(data);
    data = realloc(data, data_length + 1U);
    ck_assert_ptr_nonnull(data);
    data[data_length++] = 'A';
...
START_TEST(test_apm_partition_coordinate_overflow_is_fail_visible)
{
    ...
    fmap_t *map;
    cl_error_t ret;
    uint32_t first_block = UINT32_MAX / APM_FALLBACK_SECTOR_SIZE + 2U;

    /* On a narrow size_t build, the declared block coordinate would wrap
     * into the readable third block unless conversion is checked before the
     * nested scan is entered. */
    data[0] = 0x45;
    data[1] = 0x52; /* DDM signature: "ER". */
    data[2] = 0x02;
    data[3] = 0x00; /* 512-byte blocks. */
    ...
    data[1024] = 0x50;
    data[1025] = 0x4d; /* partition entry signature: "PM". */
    data[1028] = (uint8_t)(first_block >> 24);
    data[1029] = (uint8_t)(first_block >> 16);
    data[1030] = (uint8_t)(first_block >> 8);
    data[1031] = (uint8_t)first_block;
    data[1035] = 0x01; /* one block. */
...
static size_t hwp3_paragraph_header_read_failure_offset = SIZE_MAX;

static const void *hwp3_paragraph_header_read_failure(fmap_t *map, size_t at, size_t len, int lock)
{
    (void)lock;
    if (at == hwp3_paragraph_header_read_failure_offset)
        return NULL;
    if (len == 0 || at > map->len || len > map->len - at)
        return NULL;
    return (const uint8_t *)map->data + at;
...
static size_t hwp3_paragraph_content_read_failure_offset = SIZE_MAX;

static const void *hwp3_paragraph_content_read_failure(fmap_t *map, size_t at, size_t len, int lock)
{
    (void)lock;
    if (at == hwp3_paragraph_content_read_failure_offset)
        return NULL;
    if (len == 0 || at > map->len || len > map->len - at)
        return NULL;
    return (const uint8_t *)map->data + at;
...
START_TEST(test_hwp3_truncated_paragraph_header_is_parse_error)
{
    enum {
    ...
    map = cl_fmap_open_memory(data, sizeof(data));
    ck_assert_ptr_nonnull(map);
    hwp3_paragraph_header_read_failure_offset = HWP3_PARAGRAPH_OFFSET + 1U;
    map->need                                = hwp3_paragraph_header_read_failure;
    memset(&ctx, 0, sizeof(ctx));
    memset(&options, 0, sizeof(options));
    ctx.fmap   = map;
    ctx.options = &options;
    ...
    ck_assert_int_eq(cli_scanhwp3(&ctx), CL_EPARSE);
    ck_assert(ctx.scan_incomplete);
    ck_assert_str_eq(ctx.scan_incomplete_reason, "HWP3 paragraph header could not be read completely");
    ck_assert(map->dont_cache_flag);
...
START_TEST(test_hwp3_truncated_paragraph_content_is_parse_error)
{
    enum {
        HWP3_CONTENT_OFFSET   = 30 + 128 + 1008,
        HWP3_PARAGRAPH_OFFSET = HWP3_CONTENT_OFFSET + (7 * 2) + 2,
    ...
    ck_assert_int_eq(cli_scanhwp3(&ctx), CL_EPARSE);
    ck_assert(ctx.scan_incomplete);
    ck_assert_str_eq(ctx.scan_incomplete_reason, "HWP3 paragraph content could not be read completely");
    ck_assert(map->dont_cache_flag);
...
static const void *sevenzip_sfx_read_failure(fmap_t *map, size_t at, size_t len, int lock)
{
    (void)lock;
    if (at == sevenzip_sfx_read_failure_offset && len == sevenzip_sfx_read_failure_length)
        return NULL;
    if (len == 0 || at > map->len || len > map->len - at)
        return NULL;
    return (const uint8_t *)map->data + at;
...
START_TEST(test_7z_sfx_header_read_failure_is_fail_visible)
{
    uint8_t data[33] = {0};
    struct cl_scan_options options;
    struct cl_engine *scan_engine;
    ...
    memcpy(data + 1, "7z\xbc\xaf'\x1c", 6);
    data[7] = 0;
    data[8] = 4;
    ...
    sevenzip_sfx_read_failure_offset = 1U;
    sevenzip_sfx_read_failure_length = 32U;
    {
        fmap_t *map = cl_fmap_open_memory(data, sizeof(data));
        ck_assert_ptr_nonnull(map);
        map->need = sevenzip_sfx_read_failure;
        verdict    = CL_VERDICT_STRONG_INDICATOR;
        last_alert = "stale";
        scanned    = UINT64_MAX;
    ...
    scan_engine->sdb = saved_sdb;
    sevenzip_sfx_read_failure_offset = SIZE_MAX;
    sevenzip_sfx_read_failure_length = SIZE_MAX;
    cl_engine_free(scan_engine);
}
END_TEST
...
static const void *sevenzip_test_expire_after_first_read(fmap_t *map, size_t at, size_t len, int lock)
{
    UNUSEDPARAM(lock);

    if (sevenzip_test_expire_ctx != NULL && sevenzip_test_read_calls++ == 0 &&
        gettimeofday(&sevenzip_test_expire_ctx->time_limit, NULL) == 0)
        sevenzip_test_expire_ctx->time_limit.tv_sec--;
...
START_TEST(test_7z_input_time_limit_is_fail_visible)
{
    uint8_t data[34] = {0};
    struct cl_engine engine;
    cli_ctx ctx;
    ...
    sevenzip_test_expire_ctx = &ctx;
    sevenzip_test_read_calls = 0;
    map->need                   = sevenzip_test_expire_after_first_read;
    ret                         = cli_7unz(&ctx, 0);
    sevenzip_test_expire_ctx   = NULL;
...
START_TEST(test_autoit_version_read_failure_is_fail_visible)
{
    ...
    ck_assert(ctx.scan_incomplete);
    ck_assert_str_eq(ctx.scan_incomplete_reason, "AutoIt version byte could not be read completely");
    ck_assert(map->dont_cache_flag);
...
static size_t pe_swizz_resource_read_offset;

static const void *pe_swizz_resource_read_failure(fmap_t *map, size_t at, size_t len, int lock)
{
    (void)lock;
    if (at == pe_swizz_resource_read_offset)
        return NULL;
    if (len == 0 || at > map->len || len > map->len - at)
        return NULL;
    return (const uint8_t *)map->data + at;
...
static const void *tnef_attachment_data_read_failure(fmap_t *map, size_t at, size_t len, int lock)
{
    (void)lock;
    if (at == 15U)
        return NULL;
...
static const void *jpeg_truncated_segment_size_read_failure(fmap_t *map, size_t at, size_t len, int lock)
{
    (void)lock;
    if (at == 4U)
        return NULL;
...
static size_t jpeg_segment_probe_failure_offset = SIZE_MAX;

static const void *jpeg_segment_probe_read_failure(fmap_t *map, size_t at, size_t len, int lock)
{
    (void)lock;
    if (at == jpeg_segment_probe_failure_offset)
        return NULL;
    if (len == 0 || at > map->len || len > map->len - at)
        return NULL;
    return (const uint8_t *)map->data + at;
...
{
    static const uint8_t input[8191] = {0};
    struct cl_engine engine;
    struct cl_scan_options options;
    cli_ctx ctx;
    fmap_t *map;
...
START_TEST(test_tnef_truncated_attachment_is_parse_error)
{
    static const uint8_t input[] = {
        0x78, 0x9f, 0x3e, 0x22, /* TNEF signature */
        0x00, 0x00,             /* key */
        ...
        0x05, 0x00, 0x00, 0x00, /* declared payload length */
    ...
    ck_assert(ctx.scan_incomplete);
    ck_assert_str_eq(ctx.scan_incomplete_reason, "TNEF attachment length is outside the input");
    ck_assert(map->dont_cache_flag);
...
START_TEST(test_tnef_attachment_read_failure_is_fail_visible)
{
    static const uint8_t input[] = {
        0x78, 0x9f, 0x3e, 0x22, /* TNEF signature */
        0x00, 0x00,             /* key */
    ...
    map                   = cl_fmap_open_memory(input, sizeof(input));
    ck_assert_ptr_nonnull(map);
    map->need = tnef_attachment_data_read_failure;
    ctx.fmap  = map;

    ck_assert_int_eq(cli_tnef(tmpdir, &ctx), CL_EREAD);
    ck_assert(ctx.scan_incomplete);
    ck_assert_str_eq(ctx.scan_incomplete_reason, "TNEF attachment data could not be read completely");
    ck_assert(map->dont_cache_flag);
...
#if SIZE_MAX > UINT32_MAX
struct ole2_encryption_probe_state {
    size_t length;
...
static off_t ole2_encryption_probe_pread_cb(void *handle, void *buf, size_t count, off_t offset)
{
    struct ole2_encryption_probe_state *state = handle;
    size_t copy_length;
    ...
    if ((uint64_t)offset >= state->encryption_offset ||
        ...
        state->encryption_reads++;
    if (count > state->length - (size_t)offset)
        count = state->length - (size_t)offset;
    memset(buf, 0, count);
    if ((size_t)offset < sizeof(state->header)) {
        copy_length = MIN(count, sizeof(state->header) - (size_t)offset);
        memcpy(buf, state->header + (size_t)offset, copy_length);
    }
    return (off_t)count;
}
...
START_TEST(test_xlm_output_time_limit_is_fail_visible)
{
    static const uint8_t parent_data[] = {0};
    char hash[] = "xlm-output-time-limit";
    char file_path[PATH_MAX];
    struct cl_engine engine;
    struct cl_scan_options options;
    cli_ctx ctx;
    ...
    ck_assert_int_eq(ret, CL_ETIMEOUT);
    ck_assert(ctx.scan_incomplete);
    ck_assert_str_eq(ctx.scan_incomplete_reason, "XLM macro temporary output reached the configured time limit");
    ck_assert_uint_eq(ctx.temporary_bytes, 0);
    ck_assert(map->dont_cache_flag);
...
START_TEST(test_pe_swizzor_resource_read_failure_is_fail_visible)
{
    char file_path[PATH_MAX];
    struct cl_engine *scan_engine;
    struct cl_scan_options options;
    ...
    ck_assert_ptr_nonnull(scan_engine);
    ck_assert_int_eq(cl_engine_compile(scan_engine), CL_SUCCESS);
    scan_engine->dconf->pe |= PE_CONF_SWIZZOR;
    ...
    ck_assert_msg(peinfo.ndatadirs >= 3 && peinfo.dirs[2].Size,
                  "PE fixture has no resource directory");
    pe_swizz_resource_read_offset = cli_rawaddr(peinfo.dirs[2].VirtualAddress,
                                                peinfo.sections, peinfo.nsections, &err,
                                                map->len, peinfo.hdr_size);
    ck_assert_int_eq(err, 0);
    cli_exe_info_destroy(&peinfo);

    map->need = pe_swizz_resource_read_failure;
    memset(&layer, 0, sizeof(layer));
    memset(&ctx, 0, sizeof(ctx));
    ctx.engine               = scan_engine;
    ctx.dconf                = scan_engine->dconf;
    ...
    ck_assert_int_eq(ret, CL_EREAD);
    ck_assert(ctx.scan_incomplete);
    ck_assert_str_eq(ctx.scan_incomplete_reason, "PE Swizzor resource could not be read completely");
    ck_assert(map->dont_cache_flag);
...
{
    (void)lock;
    if (at == pe_nspack_read_failure_offset)
        return NULL;
    if (len == 0 || at > map->len || len > map->len - at)
        return NULL;
    return (const uint8_t *)map->data + at;
...
START_TEST(test_pe_nspack_loader_read_failure_is_fail_visible)
{
    static const uint8_t nspack_entry[] = {
        0x9c, 0x60, 0xe8, 0x00, 0x00, 0x00, 0x00, 0x5d, 0xb8, 0x07, 0x00, 0x00, 0x00};
    char file_path[PATH_MAX];
    struct cl_engine *scan_engine;
    struct cl_scan_options options;
    struct cli_exe_info peinfo;
    ...
    cli_exe_info_init(&peinfo, 0);
    ck_assert_int_eq(cli_peheader(&header_ctx, &peinfo, CLI_PEHEADER_OPT_NONE), CL_SUCCESS);
    ck_assert_msg(!peinfo.is_pe32plus, "NsPack regression requires a PE32 fixture");
    ep = (size_t)peinfo.ep;
     ...
     * point, where the injected fmap callback will fail. */
    cli_writeint32(data + ep + 17U, 0x50U);
    pe_nspack_read_failure_offset = ep - 4U;
    cli_exe_info_destroy(&peinfo);
    ...
    ck_assert_int_eq(ret, CL_EREAD);
    ck_assert(ctx.scan_incomplete);
    ck_assert_str_eq(ctx.scan_incomplete_reason, "PE NsPack loader metadata could not be read completely");
    ck_assert(map->dont_cache_flag);
...
{
    static const unsigned char script[] = "var timeout_marker = 1;";
    struct cl_engine *scan_engine;
    struct cl_scan_options options;
    cli_scan_layer_t layer;
    cli_ctx ctx;
    ...
    ck_assert_int_eq(cli_magic_scan(&ctx, CL_TYPE_SCRIPT), CL_ETIMEOUT);
    ck_assert(ctx.scan_timed_out);
    ck_assert(ctx.scan_incomplete);
    ck_assert_str_eq(ctx.scan_incomplete_reason,
...
START_TEST(test_elf_truncated_program_header_is_parse_error)
{
    uint8_t data[64] = {0};
    struct cl_engine engine;
    struct cl_scan_options options;
    ...
    zip_stream_write_u64(data + 32, 63U);
    ...
    map = cl_fmap_open_memory(data, sizeof(data));
    ck_assert_ptr_nonnull(map);
    map->need   = elf_truncated_program_header_read_failure;
    ctx.engine  = &engine;
    ctx.options = &options;
    ctx.fmap    = map;

    /* The requested 56-byte program header starts at byte 63, so only its
     * first byte is in the map. A callback failure for that prefix must not
     * turn the already-proven truncation into CL_EREAD. */
    ret = cli_scanelf(&ctx);
    ck_assert_int_eq(ret, CL_EPARSE);
    ck_assert(ctx.scan_incomplete);
    ck_assert(map->dont_cache_flag);
...
START_TEST(test_hfsplus_temporary_directory_failure_is_fail_visible)
{
    ...
    ck_assert_msg(snprintf(invalid_tmpdir, sizeof(invalid_tmpdir), "%s/hfsplus-output-root-does-not-exist", tmpdir) < (int)sizeof(invalid_tmpdir),
                  "temporary directory test path was truncated");
    ...
    ck_assert_int_eq(ret, CL_ETMPDIR);
    ck_assert(ctx.scan_incomplete);
    ck_assert_str_eq(ctx.scan_incomplete_reason, "HFS+ temporary directory could not be created");
    ck_assert(map->dont_cache_flag);
...
START_TEST(test_jpeg_truncated_segment_size_is_parse_error)
{
    ...
    map = cl_fmap_open_memory(data, sizeof(data));
    ck_assert_ptr_nonnull(map);
    map->need = jpeg_truncated_segment_size_read_failure;
    ctx.fmap   = map;
...
START_TEST(test_jpeg_segment_probe_read_failure_is_fail_visible)
{
    static const uint8_t data[] = {
        0xff, 0xd8, 0xff, 0xe0, 0x00, 0x07,
        'J',  'F',  'I',  'F',  '\0',
    };
    cli_ctx ctx;
    fmap_t *map;
    ...
    map = cl_fmap_open_memory(data, sizeof(data));
    ck_assert_ptr_nonnull(map);
    jpeg_segment_probe_failure_offset = 6U;
    map->need                           = jpeg_segment_probe_read_failure;
    ctx.fmap                            = map;

    ck_assert_int_eq(cli_parsejpeg(&ctx), CL_EREAD);
    ck_assert(ctx.scan_incomplete);
    ck_assert_str_eq(ctx.scan_incomplete_reason, "Heuristics.Broken.Media.JPEG.ApplicationMarkerRead");
    ck_assert(map->dont_cache_flag);

    jpeg_segment_probe_failure_offset = SIZE_MAX;
    cl_fmap_close(map);
}
END_TEST
...
static size_t pe_icon_read_failure_offset = SIZE_MAX;

static const void *pe_icon_targeted_read_failure(fmap_t *map, size_t at, size_t len, int lock)
{
    (void)lock;
    if (at == pe_icon_read_failure_offset)
        return NULL;
    if (len == 0 || at > map->len || len > map->len - at)
        return NULL;
    return (const uint8_t *)map->data + at;
...
START_TEST(test_pe_icon_bitmap_header_read_failure_is_fail_visible)
{
    uint8_t data[1024];
    struct cl_engine engine;
    struct icon_matcher matcher;
    ...
    /* Resource tree: type 3 (icon) and type 14 (icon group), each with one
     * language entry. The bitmap header is fully in-range but its backing
     * fmap read is injected to fail. */
    pe_icon_test_write_u16(data + 14, 2);
    pe_icon_test_write_u32(data + 16, 3);
    pe_icon_test_write_u32(data + 20, 0x80000020U);
    pe_icon_test_write_u32(data + 24, 14);
    ...
    map = cl_fmap_open_memory(data, sizeof(data));
    ck_assert_ptr_nonnull(map);
    pe_icon_read_failure_offset = 0x100U;
    map->need                    = pe_icon_targeted_read_failure;
    ctx.engine                   = &engine;
    ctx.fmap                     = map;
    cli_icongroupset_init(&iconset);
    ...
    ck_assert_int_eq(cli_scanicon(&iconset, &ctx, &peinfo), CL_EREAD);
    ck_assert(ctx.scan_incomplete);
    ck_assert_str_eq(ctx.scan_incomplete_reason, "PE icon bitmap header could not be read completely");
    ck_assert(map->dont_cache_flag);
...
static Suite *test_cl_suite(void)
{
    ...
    tcase_add_test(tc_cl, test_tnef_truncated_attachment_is_parse_error);
    tcase_add_test(tc_cl, test_tnef_attachment_read_failure_is_fail_visible);
    ...
    tcase_add_test(tc_cl, test_xlm_output_time_limit_is_fail_visible);

...
START_TEST(test_large_document_parser_caps_are_fail_visible)
{
    ...
    memset(&map, 0, sizeof(map));
    ctx.fmap = &map;
    map.len  = XDP_DEEP_PARSE_MAX_SIZE + 1;
    ret      = cli_scanxdp(&ctx);
    ck_assert_msg(ret == CL_EPARSE, "oversized XDP returned %d", ret);
    ck_assert(ctx.scan_incomplete);
    ck_assert(map.dont_cache_flag);
...
START_TEST(test_onenote_dispatch_honors_document_dconf)
{
    ...
    /* The parser must be skipped when the document dynamic-configuration
     * bit is disabled, even though the ordinary ScanOneNote parse option is
     * requested. */

...
START_TEST(test_pcre_subject_limit_is_fail_visible)
{
    ...
    ret = cli_pcre_check_size_limit(&ctx, 0, (uint64_t)CLI_MAX_ALLOCATION);
    ck_assert_int_eq(ret, CL_SUCCESS);
    ret = cli_pcre_check_size_limit(&ctx, 0, (uint64_t)CLI_MAX_ALLOCATION + 1);
    ck_assert_int_eq(ret, CL_EMAXSIZE);
    ck_assert(ctx.scan_incomplete);
    ...
    parent.dont_cache_flag     = false;
    child.dont_cache_flag      = false;
    ret = cli_pcre_check_size_limit(&ctx, CLI_MAX_LARGE_FILESIZE, (uint64_t)CLI_MAX_ALLOCATION + 1);
    ck_assert_int_eq(ret, CL_EMAXSIZE);
    ck_assert(ctx.scan_incomplete);

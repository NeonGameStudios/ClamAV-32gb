...
START_TEST(test_hash_table_size_rejects_product_wrap)
{
    size_t bytes;

    ck_assert_int_eq(cli_hm_table_size(0, sizeof(uint8_t), &bytes), CL_SUCCESS);
    ck_assert_uint_eq(bytes, 0);
    ck_assert_int_eq(cli_hm_table_size(CLI_MAX_ALLOCATION / sizeof(uint8_t) + 1,
                                       sizeof(uint8_t), &bytes),
                     CL_ERESOURCE);
    ck_assert_int_eq(cli_hm_table_size(SIZE_MAX, sizeof(uint8_t), &bytes), CL_ERESOURCE);
    ck_assert_int_eq(cli_hm_table_size(1, 0, &bytes), CL_EARG);
    ck_assert_int_eq(cli_hm_table_size(1, sizeof(uint8_t), NULL), CL_EARG);
    ck_assert_int_eq(cli_hm_table_size(UINT32_MAX, sizeof(uint64_t), &bytes), CL_ERESOURCE);
}
END_TEST
...
START_TEST(test_logical_malformed_expression_is_fail_visible)
{
    ...
    lsig.type        = CLI_LSIG_NORMAL;
    lsig.u.logic     = logic;
    lsig.virname     = (char *)"OutOfRangeLogicalSubsignature";
    lsig.tdb.subsigs = 1;
    lsigtable[0]     = &lsig;
    root.ac_lsigs       = 1;
    root.ac_lsigtable   = lsigtable;
...
START_TEST(test_logical_expression_trailing_data_is_fail_visible)
{
    static char logic[] = "0x";
    struct cli_ac_lsig lsig;
    struct cli_ac_lsig *lsigtable[1];
    struct cli_matcher root;
    struct cli_ac_data mdata;
    ...
    lsig.type        = CLI_LSIG_NORMAL;
    lsig.u.logic     = logic;
    lsig.virname     = (char *)"TrailingLogicalExpressionData";
    lsig.tdb.subsigs = 1;
    lsigtable[0]     = &lsig;
    root.ac_lsigs       = 1;
    root.ac_lsigtable   = lsigtable;
...
START_TEST(test_logical_expression_chained_modifier_is_fail_visible)
{
    static char logic[] = "0>1>2";
    struct cli_ac_lsig lsig;
    struct cli_ac_lsig *lsigtable[1];
    struct cli_matcher root;
    struct cli_ac_data mdata;
    ...
    lsig.type        = CLI_LSIG_NORMAL;
    lsig.u.logic     = logic;
    lsig.virname     = (char *)"ChainedLogicalModifier";
    lsig.tdb.subsigs = 1;
    lsigtable[0]     = &lsig;
    root.ac_lsigs    = 1;
    root.ac_lsigtable = lsigtable;
...
START_TEST(test_logical_subsignature_bounds_are_fail_visible)
{
    static char logic[] = "1";
    struct cli_ac_lsig lsig;
    struct cli_ac_lsig *lsigtable[1];
...
START_TEST(test_logical_oversized_definition_is_fail_visible)
{
    static char logic[] = "0";
    struct cli_ac_lsig lsig;
    struct cli_ac_lsig *lsigtable[1];
    ...
    lsig.type        = CLI_LSIG_NORMAL;
    lsig.u.logic     = logic;
    lsig.virname     = (char *)"OversizedLogicalDefinition";
    lsig.tdb.subsigs = MAX_LDB_SUBSIGS + 1;
    lsigtable[0]     = &lsig;
    root.ac_lsigs    = 1;
    root.ac_lsigtable = lsigtable;
...
START_TEST(test_yara_missing_code_is_fail_visible)
{
#ifdef HAVE_YARA
    struct cli_ac_lsig lsig;
    struct cli_ac_lsig *lsigtable[1];
    ...
    lsig.id             = 0;
    lsig.type           = CLI_YARA_NORMAL;
    lsig.virname        = (char *)"MissingYaraInstructionStream";
    lsigtable[0]        = &lsig;
    root.ac_lsigs       = 1;
    root.ac_lsigtable   = lsigtable;
...
{
#ifdef HAVE_YARA
    static uint8_t code[] = {OP_POP, OP_HALT};
    struct cli_ac_lsig lsig;
    struct cli_ac_lsig *lsigtable[1];
    struct cli_matcher root;
    fmap_t *map;
...
START_TEST(test_yara_invalid_memory_index_is_fail_visible)
{
#ifdef HAVE_YARA
    static uint8_t code[1 + sizeof(uint64_t) + 1];
    const uint64_t invalid_index = MAX_LOOP_NESTING * LOOP_LOCAL_VARS;
    struct cli_ac_lsig lsig;
    struct cli_ac_lsig *lsigtable[1];
    struct cli_matcher root;
    fmap_t *map;
    ...
    code[0] = OP_CLEAR_M;
    memcpy(code + 1, &invalid_index, sizeof(invalid_index));
    code[sizeof(code) - 1] = OP_HALT;
    memset(&lsig, 0, sizeof(lsig));
    memset(&root, 0, sizeof(root));
    lsig.id           = 0;
...
START_TEST(test_yara_call_operand_count_is_fail_visible)
{
#ifdef HAVE_YARA
    static uint8_t code[1 + sizeof(uint64_t) + 1];
    const uint64_t argument_count = MAX_FUNCTION_ARGS + 1;
    struct cli_ac_lsig lsig;
    struct cli_ac_lsig *lsigtable[1];
    struct cli_matcher root;
    fmap_t *map;
    ...
    code[0] = OP_CALL;
    memcpy(code + 1, &argument_count, sizeof(argument_count));
    code[sizeof(code) - 1] = OP_HALT;
    memset(&lsig, 0, sizeof(lsig));
    memset(&root, 0, sizeof(root));
    lsig.id           = 0;

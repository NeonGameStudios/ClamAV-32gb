...
## Current Mach-O focused audit — 2026-09-04

The authoritative Mach-O, matcher, scanner, and test sources were evaluated in
the existing production-linked GCC harness. Their synchronized SHA-256 values
are `macho.c`
...
The initial `macho_fat` failure was a fixture isolation defect: its direct
engine setup used the shared `tmpdir` without the `cl_setup` fixture. The TCase
now installs the checked temporary-directory fixture, and the complete
three-case run passes. No Mach-O production-code change was required. This
remains focused current-source evidence, not final Mach-O/FAT qualification.
Complete executable corpus, sanitizer, certified Linux x86-64,
production-CVD/service, materialized-large-file, resource, Sonic1, and final
PLAN.md qualification remain required.
...
and `jpeg_corpus` 1/1, with zero failures and errors. The map case covers
admission, truncation, sticky incomplete state, timeout, direct and public
in-range read failures, segment and exploit probes, Photoshop resource bounds,
and entropy completion/failure handling. The corpus case reaches the exact
nested MZ marker through a complete APP13 Photoshop resource fixture. The
manifest's earlier 13/13 count was stale; no JPEG production-code change was
required. This remains focused current-source evidence, not final JPEG/image
qualification. Full corpus, sanitizer, certified Linux x86-64,
production-CVD/service, materialized-large-file, resource, Sonic1, and final
PLAN.md qualification remain required.
...
## Current HWPOLE2 focused audit — 2026-09-04
...
zero failures and errors. The map case covers null-context, missing-map,
missing-engine/options, pre-tainted clean completion, and in-range public-API
prefix-read admission; the corpus case reaches the exact nested
`HWPOLE2.Member.MZ.UNOFFICIAL` marker through the materialized `clam.ppt`
wrapper. The pre-tainted child handoff is isolated through the Linux static
nested-fmap linker wrapper, matching HWPOLE2's actual call path.

These are focused current-source results, not final HWPOLE2 qualification.
Complete HWPOLE2/OLE2 corpus, full C/Rust ABI, sanitizer, certified Linux
...
0 qualified rows and 583 release-blocking bounded, pending, or required-
...
while establishing `MaxScanTime`. Because the zero-initialized deadline made
later checks no-ops, a scan could run without its configured deadline. The
failure now returns `CL_ERESOURCE`, marks the root incomplete and
non-cacheable, and stops before parser work. The Linux static production-linked
regression injects the clock failure through the existing linker-wrapper
harness and checks the status, cleared outputs, and fmap cache taint. Full
current-source, sanitizer, service, production-CVD, materialized-large-file,
resource, Sonic1, and final PLAN.md qualification remain open.
...
and 21 deliberate unsupported rows; 583 rows remain release-blocking under
...
Matched logical signatures with `HandlerType` now validate the current
recursion stack before reading layer attributes, returning an explicit
incomplete parse result for malformed matcher contexts. The direct matcher
...
Deferred MIME body spools retain their active scan owner in `temporary_ctx`
while they are built. `fileblobScan()` now adopts that owner before reporting
incomplete materialization or starting the authoritative scan, and rejects a
genuinely context-free scan with `CL_ENULLARG` instead of returning clean.
The focused regression covers the deferred owner and stored write error;
complete MIME/fileblob corpus, sanitizer, production-CVD/service,
...
The `fmap_gets()` line-reader wrapper now validates its map, callback,
destination, offset pointer, and destination size before dispatch, returning
`NULL` for invalid arguments. The direct fmap regression covers the boundary;
linked fmap/line-oriented parser corpus, sanitizer, service,
materialized-large-file, Sonic1, resource, and final qualification remain
open.
...
The bytecode ABI entry points now reject missing contexts, invalid PDF object
tables, missing PE metadata, null output buffers, invalid extraction cursors,
and unsafe string lengths before dereference. Legacy seek arithmetic is checked
for signed overflow, and the numeric helper now advances and bounds-checks its
input. The direct invalid-context regression covers the boundary; current
source production-GCC relink/execution, complete bytecode/YARA corpus,
sanitizer, service, materialized-large-file, Sonic1, resource, and final
qualification remain open.
...
PDF metadata for JavaScript objects, malformed-object lists, JBIG2 objects, and
large-color objects now shares a checked recorder for PDFStats/array creation
and integer append failures. A rejected required record marks the confirmed PDF
layer incomplete and non-cacheable, and its `CL_EMEM` status survives extraction,
temporary cleanup, and the final parser merge without replacing a stronger
parser or detection result.

`test_pdf_derived_metadata_array_add_failure_is_fail_visible` injects the
production-linked JavaScript-object array append failure and requires `CL_EMEM`,
the exact `PDF derived-object metadata could not be recorded` diagnostic, and
fmap cache taint. Current-source production-GCC compilation, production-linked
...
clean success to `CL_EPARSE` when the owning layer is already incomplete. The
direct regression and source guards cover the admission boundary.
Current-source production-GCC compilation, production-linked execution,
sanitizer, complete PDF/embedded-document corpus, certified Linux x86-64,
production-CVD/service, materialized-large-file, Sonic1, resource, and final
parser/release qualification remain required.
...
clean completion against the owning context’s sticky incomplete state. A
valid header in a pre-tainted context returns `CL_EPARSE` while retaining the
original reason and fmap non-cacheability; stronger format and read statuses
remain authoritative. The direct regression and source guards cover the
...
Confirmed AutoIt SFX header admission now reconciles clean completion against
the owning context’s sticky incomplete state. A valid header in a pre-tainted
context returns `CL_EPARSE` while retaining the original reason and fmap
non-cacheability; stronger format, read, and parser statuses remain
authoritative. The direct regression and source guards cover both confirmed
header-success paths. Current-source production-GCC compilation,
production-linked execution, sanitizer, complete AutoIt corpus, certified
Linux x86-64, production-CVD/service, materialized-large-file, Sonic1,
resource, and final parser/release qualification remain required.
...
owning context’s sticky incomplete state. A valid bounded SFX header in a
...
not final AutoIt qualification. The EA05 format-defined 4 GiB boundary,
...
This refresh confirms bounded InstallShield admission and SFX handoff, not
final InstallShield qualification. Complete MSI/legacy/CAB corpus, sanitizer,
certified Linux x86-64, production-CVD/service, materialized-large-file,
Sonic1, and final parser/release qualification remain required.
...
This refresh confirms bounded HWPOLE2/OLE2 wrapper evidence, not final
HWPOLE2 qualification. Complete HWPOLE2/OLE2 corpus, sanitizer, certified
Linux x86-64, production-CVD/service, materialized-large-file, Sonic1, and
final parser/release qualification remain required.
...
The canonical `libmspack.c`, `libmspack.h`, `scanners.c`, and `check_clamav.c`
sources were transferred into the existing Docker production-linked GCC
harness with exact SHA-256 equality. The repository CMake targets materialized
`clam.cab` (`489ffd1f819cf09d742de715a20f4e0cc6b8d50bc837452b59f5b09304e961a5`)
and `clam.chm`
(`f22f10a9fa67f984589d85db753b83e1e3dd6780aa9f425d0fa27fd545d9bd7e`). The
current-source focused cases pass `mspack_map` 6/6 and `mspack` 7/7, including
callback and constructor failures, timeout, declared-output-size checks, and
exact nested MZP detection through both materialized CAB and CHM members.
Complete CAB/CHM corpus, sanitizer, certified Linux x86-64,
...
The canonical `special.c`, `special.h`, `scanners.c`, and `check_clamav.c`
sources were transferred into the existing Docker production-linked GCC
harness with exact SHA-256 equality (`special.c`
`39b2f780cb755f200ad3af684dda23023d7ede6271c33fc29d131ec286747aed`,
`special.h` `f729937e4464a60df1d05fa5157d59aedf30aca4fefdb221f972cef2fabf2890`,
`scanners.c` `be01859e77f89b36e337fa284a093c77be9ee17f6ba91add26182df6dcf05565`,
and `check_clamav.c`
`d4e4c4529d239da0dc82bd963ae571316c3abe922743a9cce9f834a0d546a380`). The
current-source production-linked GCC harness passes `riff` 8/8, `riff_map` 1/1,
and `riff_corpus` 1/1. The corpus is a valid ACON RIFF with a bounded nested
LIST and child chunk; direct traversal reaches the declared boundary and the
typed scan reaches the exact fixed-offset nested matcher. Complete RIFF/member
corpus, sanitizer scanner fault injection, certified Linux x86-64,
production-CVD/service, materialized-large-file, Sonic1, resource, and final
parser/release qualification remain required.
...
`check_clamav.c`
`d4e4c4529d239da0dc82bd963ae571316c3abe922743a9cce9f834a0d546a380`). The
current-source production-linked GCC harness passes `screnc` 1/1 and `script`
1/1 for their public in-range callback-failure paths, preserving fail-visible
incomplete results. Complete SCRENC/script corpora, sanitizer scanner fault
injection, certified Linux x86-64, production-CVD/service, materialized-large-
file, Sonic1, resource, and final parser/release qualification remain required.

## Current SIS production-linked rerun — 2026-08-31

The canonical `sis.c`, `sis.h`, `scanners.c`, and `check_clamav.c` sources were
transferred into the existing Docker production-linked GCC harness with exact
SHA-256 equality (`sis.c`
`7833212547d3a8048e44e10c75a5089604d53c41d0674877b0ed90c96483013c`, `sis.h`
`214eac48469c2140ee14439575f629a3267965369e1b4fc7494a7a29355e5668`,
`scanners.c` `be01859e77f89b36e337fa284a093c77be9ee17f6ba91add26182df6dcf05565`,
and `check_clamav.c`
`d4e4c4529d239da0dc82bd963ae571316c3abe922743a9cce9f834a0d546a380`). The
...
`hfs_fork` 1/1. Complete HFS+ corpus including ExtentOverflow records,
...
This refresh confirms generic graphics dispatch evidence, not final graphics
or image-family qualification. Complete graphics/image corpora, sanitizer,
certified Linux x86-64, production-CVD/service, materialized-large-file,
Sonic1, and final parser/release qualification remain required.
...
This refresh confirms bounded GPT parser and corpus evidence, not final GPT
qualification. Complete GPT/partition-image corpus, sanitizer, certified Linux
x86-64, production-CVD/service, materialized-large-file, Sonic1, and final
parser/release qualification remain required.
...
leak detection. Full HFS+ corpus, including ExtentOverflow corpus coverage,
...
The compressed-resource walker now checks the addition of each untrusted
block-table offset to the current resource-data position before converting the
coordinate to native `off_t` and calling `lseek()`. Negative positions,
unsigned-addition overflow, and non-round-trippable native conversions record
a sticky incomplete, non-cacheable format result. The new header oracle covers
null, ordinary, and overflowing additions; full production-linked HFS+ corpus,
sanitizer, resource-measurement, production-CVD/service, materialized
large-file, Sonic1, and final HFS+ qualification remain required.
...
the architecture class: command sizes must be multiples of four for 32-bit
images and eight for 64-bit images. Misaligned commands fail before
command-specific parsing, mark the layer incomplete, and prevent clean-result
caching. The current-source production-linked GCC fixture passes 2/2 for
malformed 32-bit and 64-bit command sizes; complete Mach-O corpus, sanitizer,
certified Linux x86-64, materialized large-file, production-CVD/service,
Sonic1, and parser-family qualification remain open.
...
The bundled YARA interpreter previously asserted on an unknown opcode, which
could abort a debug build instead of producing a visible result for malformed
bytecode. Unknown opcodes now return `CL_EPARSE`; the YARA matcher converts that
to an incomplete, non-cacheable result. A focused current-source GCC
section-garbage-collected harness passes the unknown-opcode check. The full
production-linked matcher TCase, sanitizer, production-CVD/service, Sonic1,
...
header ID, and zero reserved field, matching embedded EGG-SFX admission.
Malformed fields return `CL_EPARSE`, mark the context incomplete, and keep the
fmap non-cacheable. The current-source parser builds warning-clean with GCC
`-Wall -Wextra -Wformat-security`; production-linked `egg_map`,
`egg_metadata`, and `egg_sfx` pass 6/6, 1/1, and 1/1. Full EGG corpus,
sanitizer, certified Linux x86-64, materialized large-file,
production-CVD/service, Sonic1, and parser-family qualification remain open.
...
when its architecture-count field was at least 39, silently skipping
...
sources matching the Docker build inputs, passes `onenote` 2/2,
`rust_onenote` 2/2, and `rust_map` 1/1. The materialized OneNote corpus
reaches the exact nested MZP matcher, while the Rust boundary and map cases
retain their read/truncation and missing-map checks. The earlier CL_EPARSE
comparison was a stale or mixed C/Rust linkage artifact. Full OneNote/OOXML
corpus, sanitizer, certified Linux x86-64, materialized large-file,
production-CVD/service, Sonic1, and release qualification remain open.
...
The authoritative current-source production-linked GCC rebuild, with a fresh
`msexpand.c` object ahead of the existing static helper archive, passes the
complete `msexpand` TCase 6/6 and `msexpand_map` 2/2. It covers null-context
and missing-map admission, fixed-header truncation/read failure, output and
temporary limits, timeout, and materialized `clam.exe.szdd` decompression with
exact nested-child matching. The earlier 4/6 and 5/6 results were stale or
mixed helper-linkage artifacts. Complete SZDD corpus, sanitizer, certified
Linux x86-64, materialized large-file, production-CVD/service, Sonic1, and
release qualification remain open.
...
a child. A clean current-source production-linked GCC relink of the test,
...
result. Full RTF corpus, sanitizer, certified Linux x86-64, materialized
large-file, production-CVD/service, Sonic1, and release qualification remain
open.
...
4/4 using direct compiled matcher setup. It covers exact nested member
matching, checksum-mismatch precedence, a tail marker after more than two
64 KiB checksum windows, and an injected checksum read failure. The existing
`cpio` corpus passes 1/1, `cpio_map` 4/4, and `cpio_numeric` 1/1. Full CPIO
corpus, sanitizer, certified Linux x86-64, materialized large-file,
production-CVD/service, Sonic1, and release qualification remain open.
...
prefixed CAB SFX is admitted through public `CL_TYPE_CABSFX` dispatch and its
extracted child reaches an exact offset-0 matcher. Full CAB-SFX corpus,
sanitizer, certified Linux x86-64, materialized large-file,
production-CVD/service, Sonic1, and release qualification remain open.
...
1/1 over a 65,556-byte synthetic binary payload whose exact-tail custom matcher
is reached through public `CL_TYPE_BINARY_DATA` dispatch. This proves the
no-parser binary-data branch retains the generic raw matcher through the end
of the fmap. Broader hash/AC/BM/logical/YARA coverage, ingress parity,
sanitizer, certified Linux x86-64, materialized large-file,
production-CVD/service, Sonic1, and release qualification remain open.
...
1/1 across both public `CL_TYPE_XML_WORD` and `CL_TYPE_XML_XL` dispatch paths
over a complete XML document. Its newline-delimited Base64 `bindata` member
reaches the exact `MSXML.Member.Marker.UNOFFICIAL` matcher after legacy MSXML
temporary materialization and nested output scanning; the outer XML does not
contain the marker. Full MSXML/XML corpus, sanitizer, certified Linux x86-64,
materialized large-file, production-CVD/service, Sonic1, and release
qualification remain open.
...
2/2, and the isolated apm_corpus case passes 1/1. It uses a valid four-block
Apple Partition Map with a driver map, partition-map entry, and one payload
partition; the APM root does not begin with MZP, and the exact
Apm.Partition.MZ.UNOFFICIAL alert is reached through partition traversal.
Full APM partition corpus, sanitizer, certified Linux x86-64, materialized
large-file, production-CVD/service, Sonic1, and release qualification remain
open.
...
Fresh remote SSH verification with `<redacted-login-profile>` used host-list request
`<redacted-request-id>`, Docker status request
`<redacted-request-id>`, inspect request
`<redacted-request-id>`, and source-hash request
`<redacted-request-id>`. Docker was reachable; all three
...
Fresh remote SSH verification used `<redacted-login-profile>`: host list
...
`<redacted-request-id>`. All three existing containers were
...
`<redacted-request-id>`. All three containers were up on
...
Fresh remote SSH verification used `<redacted-login-profile>`: host-list request
`<redacted-request-id>`, Docker status request
`<redacted-request-id>`, and container-hash request
`<redacted-request-id>`. All three validation containers were
...
Sonic1 Docker liveness was refreshed with `<redacted-login-profile>`; all three
...
Sonic1 Docker liveness was refreshed with `<redacted-login-profile>`; its current
...
HFS+ fork records that exhaust their eight inline extents now resolve the
required `ExtentOverflow` B-tree records. The resolver binds each record to the
fork type, catalog file ID, and logical starting block, validates the leaf
chain, record offsets, descriptor ranges, volume coordinates, and scan
deadline, then reads the mapped physical blocks through the existing fmap
path. The ExtentOverflow file itself is intentionally limited to its inline
fork; recursive overflow is rejected as malformed rather than followed
without a bound. HFS+ resource compression remains implemented through the
bounded temporary-fork path; its completion diagnostic is not an unsupported-
feature indication.
...
## PDF native-width LZW filter chains — 2026-09-14

The supported PDF filter-chain reader already processes LZW through fixed-size
input windows and native-width source coordinates, but top-level admission was
still rejecting every multi-filter chain containing LZW when the declared input
length exceeded 4 GiB or the contiguous allocation ceiling. That stale guard is
now removed for supported chains, so LZW can use the same bounded, transactional
file-backed stage rotation as Flate, RunLength, ASCIIHex, and ASCII85. Unsupported
or mixed chains still take the legacy path and retain its explicit 32-bit and
allocation-boundary rejection.

The current-source ARM64 production-linked `cl_api` regression passes 529/529
under both Release and ASan/UBSan, including a valid ASCIIHex-to-LZW chain with a
logical input length above 4 GiB and the preserved legacy-boundary rejection for
an unsupported mixed chain. No sanitizer diagnostics were emitted. This is
focused development evidence; materialized multi-gigabyte PDF streams,
certified Linux x86-64, production CVD/service, resource, Sonic1, and final
parser/release qualification remain open.

## ENABLE_WERROR flag propagation — 2026-09-14

The CMake warning-flag helper now preserves the flags already accumulated in the
requested variable when a later call appends more flags. This makes
`ENABLE_WERROR=ON` effective in the generated `WARNCFLAGS` and `WARNCXXFLAGS`
instead of allowing the later `-Wall`/`-Wextra` calls to discard `-Werror`.

An isolated ARM64 Release configuration reports `-Werror -Wall -Wextra
-Wformat-security` in `WARNCFLAGS`. The focused `regex`, `lzma_sdk`, and `yara`
targets now compile cleanly, as do the fileblob, AC/BM matcher, TFLite, and
InstallShield source objects reached by the library build. The next global
blocker is the legacy NSIS bzip2 state machine's intentional macro-driven
fallthrough diagnostics; this slice therefore verifies option propagation and
several warning-clean source families, but does not claim global warning-clean
qualification or promote `ENABLE_WERROR`.

## ENABLE_WERROR application build closure — 2026-09-14

The follow-up warning cleanup completed the configured current-source Werror
application build, including the legacy NSIS bzip2 state machine and the
bundled regex, LZMA/XZ, YARA, matcher, TFLite, InstallShield, daemon, milter,
and Rust-linked application paths. Release `cl_api` passes 529/529, and focused
Release/ASAN/UBSAN parser suites pass; the current ASAN aggregate `cl_api`
rerun did not return and is not counted as a pass. Certified Linux x86-64,
production databases and service, full-size materialized inputs,
resource/on-access, Sonic1, and final release qualification remain open. A
separate maximal-warning exploration now passes `libclamav/str.c`,
`strlcat.c`, `table.c`, `www.c`, `disasm.c`, and `filtering.c` before reaching
the larger legacy warning set in `libclamav/matcher-ac.c`; it is not the
supported Werror build gate.

The revision-bound receipt is
...
current worktree transfer was not permitted by the remote SSH export policy.
...
Sonic1 verification remains open because remote SSH did not permit exporting the
...
The remote SSH host inventory and the declared `<redacted-login-profile>` capabilities
...
before returning, including malformed or missing ExtentOverflow records. Catalog
...
for complete groups, supports whitespace and `z` groups, validates `z`
placement and one-character final groups, rejects full or partial groups whose
numeric value exceeds the 32-bit ASCII85 tuple, rejects a marker-less partial
trailing group instead of silently discarding it, completes two- through
four-character groups at `~>`, and ignores post-marker data. Invalid input
after a written decoded prefix rolls back the file and reservations before
exact raw fallback.
...
The full ELF, Mach-O, and Mach-O universal-binary parser entries now reject
recognized maps without an attached engine before deadline or heuristic state
access. Engine-free Mach-O metadata parsing remains available through
cli_machoheader when a fileinfo output structure is supplied. The focused
test_executable_parsers_require_engine regression covers the full-scan
boundaries. The current ELF, Mach-O, and unit translation units pass the
established Docker GCC syntax checks; complete executable corpus, sanitizer,
production-CVD/service, materialized large-file, Sonic1, and release
qualification remain open.
...
After verifying exact canonical/container SHA-256 equality for `macho.c`
...
expected by the bundled decoder, reaches successful decompression, injects
finalization failure, and requires `CL_EUNPACK`, the exact sticky cleanup
reason, and fmap non-cacheability. The prior failure was a test-fixture format
mismatch: standard BZIP2 block-header bytes were passed to the NSIS-specific
decoder layout, so cleanup was never reached. No NSIS production parser change
was needed. This is focused NSIS/SFX evidence, not complete family
...
and `hwpml_corpus` 1/1. Coverage includes invalid rooted PartName
admission, metadata failure propagation, PowerPoint VBA boundaries, streaming
MSXML faults and deadlines, and exact decoded/Base64 nested-marker handoff.

This is focused current-source evidence, not complete OOXML qualification.
...
The current-source production-linked GCC matrix passes with zero failures and
errors: `pdf` 22/22, `pdf_map` 7/7, and `pdf_corpus` 1/1 after materializing
`clam.pdf` through the existing repository CMake target. This replaces the
older 17/17 and 4/4 focused counts and retains exact offset-0 nested-MZP
handoff after PDF extraction.

This is focused GCC parser/map/corpus evidence, not complete PDF qualification.
Complete PDF/filter corpus, production/sanitizer scanner fault injection,
certified Linux x86-64, production-CVD/service, materialized-large-file/
...
The current-source production-linked GCC matrix passes with zero failures and
errors: TNEF `tnef` 18/18, `tnef_map` 3/3, and `tnef_debug` 2/2; UDF
`udf_map` 14/14 and `udf_corpus` 1/1; and UUEncode `uuencode_map` 5/5 and
`uuencode_corpus` 1/1. These runs cover direct-entry admission, fail-visible
read/temporary/deadline boundaries, debug-dump status propagation, and exact
nested-child handoff where the focused corpus fixtures provide one.

This is focused current-source evidence, not complete TNEF/UDF/UUEncode
qualification. Complete parser-family corpora, sanitizer, certified Linux
x86-64, production-CVD/service, materialized-large-file/resource, Sonic1, and
final parser/release evidence remain required.
...
The current-source production-linked GCC matrix passes with zero failures and
errors: XAR `xar` 19/19, `xar_metadata` 3/3, `xar_map` 3/3, `xar_corpus` 3/3,
and `xar_subdoc` 1/1; XDP `xdp` 3/3, `xdp_map` 5/5, and `xdp_corpus` 1/1.
The XAR result corrects stale manifest references to the earlier `xar_map`
2/2 count. The corpus cases reach exact nested child matchers after XAR
member extraction and XDP Base64 decoding.

This is focused current-source evidence, not complete XAR/XDP qualification.
Complete archive/XML corpora, sanitizer, certified Linux x86-64,
production-CVD/service, materialized-large-file/resource, Sonic1, and final
parser/release evidence remain required.

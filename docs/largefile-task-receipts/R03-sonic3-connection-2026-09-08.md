...
- Source checkout: `/Volumes/512gbNVME/github-external/ClamAV-32gb`
...
The MCP-SSH host inventory listed `sonic1` and `sonic3`. The supplied
`sonic3-sudo` profile described successfully as a POSIX host at
`192.168.1.205:22`, with `sonic3` SFTP identity and sudo-enabled command
execution.
...
MCP-SSH matched the configured
...
login-profile name and MCP-SSH profiles are intentionally not discoverable.

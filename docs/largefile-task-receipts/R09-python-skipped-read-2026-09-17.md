# R09 Python compiled-bytecode skipped-read hardening — 2026-09-17

Task ID / parent milestone: R09 required unsupported rows; Python compiled
bytecode parser fail-closed behavior.
...
Observed failing case and expected behavior: the bounded Python marshal walk
validated skipped payload bounds by advancing the fmap offset without reading
the skipped bytes. An in-range backing-read failure inside `co_code` or another
skipped payload could therefore be accepted as a clean structural parse. The
expected result is `CL_EREAD`, an incomplete/non-cacheable scan, and no clean
verdict.

Changes made: `cli_python_skip()` now reads skipped ranges through bounded
4-KiB buffers, checks the scan deadline per chunk, and maps a backing read
failure to `CL_EREAD` with an explicit incomplete reason. A current-source C
regression builds a valid legacy code-object envelope with a 300,000-byte
skipped code payload and fails the fmap after the first 256 KiB; it is
registered in both the ordinary and required-unsupported test cases.

Commands, exits, logs and fixture hashes:

* `sh -n tools/largefile_source_guards.sh` — exit 0.
* `sh tools/largefile_inventory.sh` followed by exact `diff` against
  `docs/largefile-inventory.tsv` — exit 0.
* `git diff --check` — exit 0.
* `sh tools/largefile_source_guards.sh` — exit 0; 604 capability bindings and
  all included Python, acceptance, evidence, snapshot, and readiness checks
  passed.
...
fresh Python parser regression and R04 capability cases. Release readiness
remains blocked.

...
while IFS="$(printf '\t')" read -r file marker expected_offset expected_size kind; do
    ...
    while kill -0 "$scan_pid" 2>/dev/null; do
        current_tmp_bytes=$(du -sk "$work" 2>/dev/null | awk '{ print $1 * 1024 }')
        case "$current_tmp_bytes" in
            ''|*[!0-9]*) current_tmp_bytes=0 ;;
        esac
        if [ "$current_tmp_bytes" -gt "$peak_tmp_bytes" ]; then
            peak_tmp_bytes=$current_tmp_bytes
        fi
        sleep 0.05
    done
    ...
    # Record the observed peak while the scanner was alive. A post-exit du
    # cannot see temporary data that the scanner deleted before returning.
    tmp_bytes=$peak_tmp_bytes

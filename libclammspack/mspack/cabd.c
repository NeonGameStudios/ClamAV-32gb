                        ...
                        struct mscabd_file *file, const char *filename)
{
  ...
  /* If the file claims to go beyond the decompressor's representable
   * coordinate range, report an error even in salvage mode.  Returning a
   * truncated member would let callers scan an incomplete object and report
   * the uninspected tail as clean. */
  filelen = file->length;

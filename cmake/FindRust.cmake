...
function(add_rust_executable)
    ...
    add_custom_command(
        OUTPUT "${OUTPUT}"
        COMMAND ${CMAKE_COMMAND} -E env ${ARGS_ENVIRONMENT} "RUSTC=${rustc_EXECUTABLE}" ${cargo_EXECUTABLE} ${MY_CARGO_ARGS}
        WORKING_DIRECTORY "${ARGS_SOURCE_DIRECTORY}"
        DEPENDS ${EXE_SOURCES}
...
function(add_rust_library)
    ...
    list(APPEND ARGS_ENVIRONMENT "CARGO_CMD=build" "CARGO_TARGET_DIR=${ARGS_BINARY_DIRECTORY}" "MAINTAINER_MODE=${MAINTAINER_MODE}" "CARGO_INCLUDE_DIRECTORIES=\"${ARGS_INCLUDE_DIRECTORIES}\"" "RUSTFLAGS=${RUSTFLAGS}" "RUSTC=${rustc_EXECUTABLE}")
...
# Cargo 1.97 is the minimum supported version for this fork. Cargo and Rust
# are released together, so rust-toolchain.toml selects the same release line
# for rustup-based builds. Keep this check here for system-package and
# cross-compilation builds as well.
set(CARGO_MINIMUM_REQUIRED "1.97")
if(cargo_VERSION VERSION_LESS CARGO_MINIMUM_REQUIRED)
    message(FATAL_ERROR "Cargo ${CARGO_MINIMUM_REQUIRED} or newer is required:
    ${cargo_VERSION} < ${CARGO_MINIMUM_REQUIRED}")
endif()
set(RUSTC_MINIMUM_REQUIRED "1.97")

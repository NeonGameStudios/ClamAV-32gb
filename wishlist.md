...
- Keep milter exact-edge evidence tied to the 64-GiB logical budget, exact
  signature/offset/root-size/completion metadata, with `logical_bytes` equal
  to the exact 32-GiB root and effective `max_scan_size` equal to 64 GiB, and
  a complete-stream digest. Detection-terminated is acceptable only when the
  expected tail detection is independently proven; clean scans still require
  COMPLETE.
...
- Keep OneNote modern whole-input parsing explicitly bounded at 256 MiB until
  ...
  pending their complete corpus and release evidence.
...
- Keep ZIP bounded BZIP2 decoder teardown fail-visible before member
  completion or child dispatch, and preserve the same status in the dormant
  legacy path. Merge `CL_EUNPACK` without replacing an earlier parser, read,
  limit, timeout, detection, or cleanup result; never scan partial output.
- Retain `test_zip_bzip_decoder_finalization_failure_is_fail_visible`, its
  one-shot wrapper fault, and source guards. Current-source production-GCC
  execution, complete ZIP/BZIP2 corpus, sanitizer, certified Linux x86-64,
  production-CVD/service, materialized-large-file, Sonic1, resource, and
  final parser/release qualification remain open.

## BZIP2 scanner decoder finalization status — 2026-09-02

- Keep every initialized top-level BZIP2 decoder teardown fail-visible across
  temporary-file, output, concatenated-member, and normal-completion paths.
  Merge `CL_EUNPACK` without replacing an earlier parser, read, limit,
  timeout, detection, or cleanup result, and never scan partial output.
- Retain `test_bzip_decoder_finalization_failure_is_fail_visible`, its
  one-shot wrapper fault, and source guards. Current-source production-GCC
  execution, complete BZIP2 corpus, sanitizer, certified Linux x86-64,
  production-CVD/service, materialized-large-file, Sonic1, resource, and
  final parser/release qualification remain open.
...
- Keep exported `cli_scanarj()` fail-visible when an otherwise recognized
  engine/map context omits scan options; return `CL_ENULLARG` before temporary
  directory creation, deadline/limit checks, extraction, or nested-member
  scanning.
- Retain `test_arj_missing_options_is_fail_visible`, its source guards, and
  manifest evidence that the invalid context does not become sticky incomplete
  or cache-tainted. Keep valid options in the ARJ entry and header-read
  regressions. Current-source production-GCC compilation, production-linked
  execution, complete ARJ/ARJ-SFX corpus, sanitizer, certified Linux x86-64,
  production-CVD/service, materialized-large-file, Sonic1, resource, and
  final parser/release qualification remain open.
...
- Keep exported `cli_scanpe()` fail-visible when an otherwise recognized
  engine/map context omits scan options; return `CL_ENULLARG` before JSON
  timeout/metadata handling, parser selection, unpacking, or nested scanning.
- Retain `test_pe_missing_options_is_fail_visible`, its source guards, and
  manifest evidence that the invalid context does not become sticky incomplete
  or cache-tainted. Keep valid options in direct PE parser regressions.
  Current-source production-GCC compilation, production-linked execution,
  complete PE/unpacker corpus, sanitizer, certified Linux x86-64,
  production-CVD/service, materialized-large-file, Sonic1, resource, and
  final parser/release qualification remain open.
...
- Keep confirmed MSI and legacy InstallShield extraction entries fail-visible
  when an otherwise recognized engine/map context omits scan options; return
  `CL_ENULLARG` before temporary output, metadata traversal, or nested-child
  scanning.
- Retain `test_ishield_missing_options_confirmed_entries_are_fail_visible`,
  its source guards, and manifest evidence that the invalid context does not
  become sticky incomplete or cache-tainted. Current-source production-GCC
  compilation, production-linked execution, complete InstallShield/CAB
  corpus, sanitizer, certified Linux x86-64, production-CVD/service,
  materialized-large-file, Sonic1, resource, and final parser/release
  qualification remain open.
...
- Keep exported `cli_scanautoit()` fail-visible when an otherwise recognized
  engine/map context omits scan options; return `CL_ENULLARG` before deadline
  checks, temporary output, decoder traversal, or nested-child scanning.
- Retain `test_autoit_missing_options_is_fail_visible`, its source guards,
  and manifest evidence that the invalid context does not become sticky
  incomplete or cache-tainted. Keep valid options in direct AutoIt parser
  regressions. Current-source production-GCC compilation, production-linked
  execution, complete AutoIt corpus, sanitizer, certified Linux x86-64,
  production-CVD/service, materialized-large-file, Sonic1, resource, and
  final parser/release qualification remain open.

## HWPML direct options admission — 2026-09-02

- Keep exported `cli_scanhwpml()` fail-visible when an otherwise recognized
  engine/map context omits scan options; return `CL_ENULLARG` before JSON-mode
  streaming, deadline checks, metadata access, or nested-child scanning.
- Retain `test_hwpml_missing_options_is_fail_visible`, its source guards, and
  manifest evidence that the invalid context does not become sticky incomplete
  or cache-tainted. Current-source production-GCC compilation,
  production-linked execution, complete HWPML/XML/OOXML corpus, sanitizer,
  certified Linux x86-64, production-CVD/service, materialized-large-file,
  Sonic1, resource, and final parser/release qualification remain open.
...
- Keep exported `cli_scaniso()` fail-visible when an otherwise recognized
  engine/map context omits scan options; return `CL_ENULLARG` before ISO
  descriptor traversal or child dispatch can rely on incomplete configuration.
- Retain `test_iso_missing_options_is_fail_visible`, its source guards, and
  manifest evidence that the invalid context does not become sticky incomplete
  or cache-tainted. Current-source production-GCC compilation,
  production-linked execution, complete ISO/Joliet corpus, sanitizer,
  certified Linux x86-64, production-CVD/service, materialized-large-file,
  Sonic1, resource, and final parser/release qualification remain open.
...
- Keep exported `cli_7unz()` fail-visible when an otherwise recognized
  engine/map context omits scan options; return `CL_ENULLARG` before deadline
  checks, temporary output, archive traversal, or nested-child scanning.
- Retain `test_7z_missing_options_is_fail_visible`, its source guards, and
  manifest evidence that the invalid context does not become sticky incomplete
  or cache-tainted. Current-source production-GCC compilation,
  production-linked execution, complete 7-Zip corpus, sanitizer, certified
  Linux x86-64, production-CVD/service, materialized-large-file, Sonic1,
  resource, and final parser/release qualification remain open.
...
- Keep exported `cli_scanudf()` fail-visible when an otherwise recognized
  engine/map context omits scan options; return `CL_ENULLARG` before UDF
  time-limit checks, descriptor traversal, or nested-child scanning.
- Retain valid options in direct UDF parser regressions and keep
  `test_udf_missing_options_is_fail_visible`, its source guards, and manifest
  evidence that the invalid context does not become sticky incomplete or
  cache-tainted. Current-source production-GCC compilation, production-linked
...
- Keep confirmed `cli_ishield_msi_header_check()` success fail-visible when
  the owning SFX context is already incomplete; convert only clean completion
  to `CL_EPARSE` and preserve stronger header-probe statuses.
- Retain the direct valid-header probe in
  `test_ishield_sticky_incomplete_result_is_fail_visible`, the exact prior
  diagnostic, fmap non-cacheability, and source guards.
- Complete current-source production-GCC compilation and production-linked
  execution, sanitizer, complete InstallShield/CAB corpus, certified Linux
  x86-64, production-CVD/service, materialized-large-file, Sonic1, resource,
  and final parser/release qualification.

## AutoIt SFX header sticky completion — 2026-09-01

- Keep confirmed `cli_autoit_header_check()` success fail-visible when the
...
- Retain the direct header probe in
  `test_autoit_sticky_incomplete_result_is_fail_visible`, the exact prior
  diagnostic, fmap non-cacheability, and source guards.
- Complete current-source production-GCC compilation and production-linked
  execution, sanitizer, complete AutoIt corpus, certified Linux x86-64,
  production-CVD/service, materialized-large-file, Sonic1, resource, and
  final parser/release qualification.
...
- Keep confirmed `cli_7z_header_check()` success fail-visible when the owning
  SFX context is already incomplete; convert only clean completion to
  `CL_EPARSE` and preserve stronger header-probe statuses.
- Retain `test_7z_sfx_header_sticky_incomplete_result_is_fail_visible`, the
  exact pre-existing diagnostic, fmap non-cacheability, and source guards.
- Complete current-source production-GCC compilation and production-linked
  execution, sanitizer, complete 7-Zip/SFX corpus, certified Linux x86-64,
  production-CVD/service, materialized-large-file, Sonic1, resource, and
  final parser/release qualification.
...
- Keep `cli_scanmscab()` and `cli_scanmschm()` fail-visible for direct callers
  when valid CAB or CHM member walks complete with a pre-existing sticky
  incomplete state. Clean completion now returns `CL_EPARSE`, while detections
  and stronger decoder, I/O, timeout, limit, and cleanup errors remain
  authoritative.
- Retain `test_mspack_sticky_incomplete_result_is_fail_visible`, which uses the
  materialized `clam.cab` and `clam.chm` fixtures with an initialized
  production engine and checks the exact prior reason plus non-cacheability for
  both direct entries. The current-source production-linked GCC isolation
  runner passes clean untainted baselines and `CL_EPARSE` for both sticky
  cases, and the pre-change comparison returns clean for both. Keep full
  current MSPack TCase relink/execution, sanitizer, certified Linux x86-64,
  production-CVD/service, materialized-large-file, Sonic1, resource, and final
  parser/release qualification open.
...
- Keep `cli_scansis()` fail-visible for direct callers when a valid SIS walk
  completes with a pre-existing sticky incomplete state. Clean completion now
  returns `CL_EPARSE`, while detections and stronger parser or resource errors
  remain preserved.
- Retain `test_sis_sticky_incomplete_result_is_fail_visible`, which uses the
  materialized valid `clam.sis` fixture with nested descriptor dispatch
  isolated and checks the exact prior reason plus non-cacheability. The
  current-source production-linked GCC isolation runner passes 1/1, the
  untainted baseline is clean, and the pre-change comparison returns clean for
  the same fixture. Keep full current SIS TCase relink/execution, sanitizer,
  certified Linux x86-64, production-CVD/service, materialized-large-file,
  Sonic1, resource, and final parser/release qualification open.
...
- Keep `cli_scanhwp5_stream()` fail-visible for direct callers when a normal,
  password-protected, or decompressed stream completes with a pre-existing
  sticky incomplete state. Clean/direct results now return `CL_EPARSE`, while
  detections and stronger parser or cleanup errors remain preserved.
- Retain the valid HWP5 stream-context regression
  `test_hwp5_stream_sticky_incomplete_result_is_fail_visible`, which isolates
  the ordinary child handoff and checks the exact prior reason plus
  non-cacheability. The current-source production-linked GCC isolation runner
  passes 1/1 and the pre-change comparison returns clean for the same fixture.
  Keep full current HWP5 TCase relink/execution, sanitizer, certified Linux
  x86-64, production-CVD/service, materialized-large-file, Sonic1, resource,
  and final parser/release qualification open.
...
- Keep `cli_scanhwp3()` fail-visible for direct callers when a valid HWP3 walk
  ...
  now return `CL_EPARSE`, while detections and stronger parser or cleanup
  errors remain preserved.
- Retain the valid synthetic HWP3 pre-taint regression
  `test_hwp3_sticky_incomplete_result_is_fail_visible`, which isolates nested
  child handoff and checks the exact prior reason plus non-cacheability. The
  current-source production-linked GCC harness passes 1/1 and the pre-change
  comparison returns clean for the same valid fixture. Keep full current HWP3
  TCase relink/execution, sanitizer, certified Linux x86-64,
  production-CVD/service, materialized-large-file, Sonic1, resource, and final
  parser/release qualification open.

## XAR direct sticky-incomplete reconciliation — 2026-09-01

- Keep `cli_scanxar()` fail-visible for direct callers when a valid TOC walk
  completes with a pre-existing sticky incomplete state. Clean/direct results
  now return `CL_EPARSE`, while detections and stronger parser or cleanup
  errors remain preserved.
- Retain the valid empty-TOC pre-taint regression
  `test_xar_sticky_incomplete_result_is_fail_visible`, which isolates nested
  child handoff and checks the exact prior reason plus non-cacheability. The
  current-source production-linked GCC harness passes 1/1 and the pre-change
  comparison returns clean for the same valid fixture. Keep full current XAR
  TCase relink/execution, sanitizer, certified Linux x86-64,
  production-CVD/service, materialized-large-file, Sonic1, resource, and final
  parser/release qualification open.

## BinHex direct sticky-incomplete reconciliation — 2026-09-01

- Keep `cli_binhex()` fail-visible for direct callers when a valid decoded
  document completes with a pre-existing sticky incomplete state. Clean/direct
  results now return `CL_EPARSE`, while detections and stronger parser or
  cleanup errors remain preserved.
- Retain `test_binhex_sticky_incomplete_result_is_fail_visible`, which uses a
  valid materialized BinHex corpus path with nested handoff isolated and checks
  the exact prior reason plus non-cacheability. The current-source
  production-linked GCC isolation runner passes 1/1 and the pre-change
  comparison returns clean for the same valid fixture. Keep full current
  BinHex TCase relink/execution, sanitizer, certified Linux x86-64,
  production-CVD/service, materialized-large-file, Sonic1, resource, and final
  parser/release qualification open.
...
- Keep `cli_mbr_check2()` fail-visible for direct admission callers when a
  valid two-sector MBR completes with a pre-existing sticky incomplete state.
  Clean/direct results now return `CL_EPARSE`, while GPT classification and
  stronger parser errors remain preserved.
- Retain the valid empty-partition MBR pre-taint regression
  `test_mbr_sticky_incomplete_result_is_fail_visible`, canonical/container
  hash equality for the current `mbr.c` and `check_clamav.c`, and current
  production-linked GCC results at `mbr` 8/8 and `mbr_corpus` 1/1. Keep
...
- Retain current-source production-linked GCC evidence at `graphics_map` 2/2,
  ...
  `swf_map` 3/3, `swf_api` 1/1, and `swf_corpus` 2/2.
- Keep complete media/image corpora, sanitizer, certified Linux x86-64,
  production-CVD/service, materialized-large-file, Sonic1, resource, and
  final parser/release qualification open.
...
- Retain current-source production-linked GCC evidence at `apm_map` 3/3,
  ...
  `dmg` 7/7 across the partition/container boundary and nested-member cases.
- Keep complete partition/container corpora, sanitizer, certified Linux
  x86-64, production-CVD/service, materialized-large-file, Sonic1, resource,
  and final parser/release qualification open.
...
- Keep the Anchor Volume Descriptor Pointer at logical sector 256 as the
  ...
  sequence, selected LVD/PD metadata, type-1 partition map, FSD, root ICB,
  descriptor tags, extent lengths, and partition-relative locations.
- Retain bounded parent/descendant FID traversal, hidden-file scanning,
  cycle/visit/depth limits, shared resource/deadline accounting, exact child
  detection, and fail-visible malformed extent, pointer, map, descriptor, and
  read-failure results with production-linked GCC and sanitizer evidence.
- Complete reserve-sequence/VDP handling, variable and continued FSDs, EFE,
...
- Retain current production-linked GCC evidence at `descriptor_map` 2/2,
  ...
  hash read/finalization/close, and MSPack position-query deadline boundaries;
  keep the MSPack open callback's null-system guard.
...
- Canonical/container SHA-256 equality was verified before relinking for
  `rtf.c` (`5044131c30247c3e6c1cbf45a617b3ad5686bdd5ffb161f8200bd341f38ab4e0`),
  `rtf.h` (`53005ef3dcb587f36f3366b80476d0a19f4d10b0175298fbe726d3f3fdaf20c1`),
  `scanners.c` (`be01859e77f89b36e337fa284a093c77be9ee17f6ba91add26182df6dcf05565`),
  and `check_clamav.c`
  (`d4e4c4529d239da0dc82bd963ae571316c3abe922743a9cce9f834a0d546a380`).
- The current-source production-linked GCC harness passes `rtf_map` 13/13 and
  `rtf` 1/1, including exact nested MZP matching after RTF extraction. The
  materialized `clam.exe.rtf` fixture has SHA-256
...
## Current SCRENC and Script production-linked rerun — 2026-08-31

- Canonical/container SHA-256 equality was verified before relinking for
  `scanners.c` (`be01859e77f89b36e337fa284a093c77be9ee17f6ba91add26182df6dcf05565`)
  and `check_clamav.c`
  (`d4e4c4529d239da0dc82bd963ae571316c3abe922743a9cce9f834a0d546a380`).
- The current-source production-linked GCC harness passes `screnc` 1/1 and
  `script` 1/1 for their public in-range callback-failure paths, preserving
  fail-visible incomplete results.
- Complete SCRENC/script corpora, sanitizer scanner fault injection, certified
  Linux x86-64, production-CVD/service, materialized-large-file, Sonic1,
  resource, and final parser/release qualification remain required.
...
- Canonical/container SHA-256 equality was verified before relinking for
  ...
  and `check_clamav.c`
  (`d4e4c4529d239da0dc82bd963ae571316c3abe922743a9cce9f834a0d546a380`).
- The current-source production-linked GCC harness passes `sis_structure` 2/2,
  `sis_member` 1/1, `sis_map` 2/2, and `sis` 1/1, including exact nested MZP
  matching after compressed-member extraction. The materialized `clam.sis`
  fixture has SHA-256
  `2e75399c9f6fa7741db9ccf506de2895b3a78a3d574fa776c65f533b717555b5`.
- Complete SIS corpus, sanitizer scanner fault injection, certified Linux
  x86-64, production-CVD/service, materialized-large-file, Sonic1, resource,
  and final parser/release qualification remain required.
...
- Canonical/container SHA-256 equality was verified before relinking for
  `swf.c` (`17ed0077a34018cda2433ec9b2766228d6483250d1c2058a1eb1faa989b045f0`),
  `swf.h` (`ae57151d8e55fb3d8a616f606d362780d3cede7295bc87a6be3a53b90c22d94d`),
  `scanners.c` (`be01859e77f89b36e337fa284a093c77be9ee17f6ba91add26182df6dcf05565`),
  and `check_clamav.c`
  (`d4e4c4529d239da0dc82bd963ae571316c3abe922743a9cce9f834a0d546a380`).
- The current-source production-linked GCC harness passes `swf` 13/13,
  `swf_map` 3/3, `swf_api` 1/1, and `swf_corpus` 2/2, including compressed and
  declared-boundary overlay matching after nested output scanning.
- Complete SWF corpus, sanitizer scanner fault injection, certified Linux
  x86-64, production-CVD/service, materialized-large-file, Sonic1, resource,
  and final parser/release qualification remain required.
...
- Canonical/container SHA-256 equality was verified before relinking for
  ...
  and `check_clamav.c`
  (`d4e4c4529d239da0dc82bd963ae571316c3abe922743a9cce9f834a0d546a380`).
- The current-source production-linked GCC `structured_map` TCase passes 5/5
  across null/map/options, callback, clipped-window, and timeout boundaries.
- Complete structured-text corpus, sanitizer scanner fault injection, certified
  Linux x86-64, production-CVD/service, materialized-large-file, Sonic1,
  resource, and final parser/release qualification remain required.
...
- Keep both fmap-backed `fmap_gets()` implementations successful for
  ...
  payload is proven.
- Retain the existing fmap API regression, source guards, and the isolated
  current-source production-linked GCC `fmap_api` TCase (1/1) covering
  memory-, descriptor-, nested-, and dumped-map variants; add sanitizer,
  complete fmap/line-oriented parser corpus,
  production-CVD/service, materialized-large-file, Sonic1, and final release
  qualification evidence.
...
- Keep `u16_normalize_tobuffer()` fail-closed for null or sub-two-byte
  destinations before `dst_size - 1`, and keep `cli_utf16toascii()` and
  `cli_utf16_to_utf8()` fail-closed for null non-empty input before reading or
  allocating.
...
- Keep evidence-store creation and indicator insertion failures fail-visible;
  mark the detected layer incomplete and non-cacheable while preserving the
  stronger error result and any evidence state already retained.
- Retain `test_indicator_evidence_add_failure_is_fail_visible` and its source
  guards. Current-source production-linked execution, complete
  signature/evidence corpus, sanitizer, certified Linux x86-64,
  production-CVD/service, materialized-large-file, Sonic1, resource, and
  final matcher/release qualification remain open.
...
- Keep child `Indicators` and `Alerts` copies fail-visible when parent-array
  insertion fails; release rejected copies, mark child and containing fmaps
  non-cacheable, and preserve the exact nested metadata diagnostic.
- Retain `test_nested_indicator_metadata_array_copy_failure_is_fail_visible`
  and its source guards. Current-source production-linked execution, complete
  signature/evidence corpus, sanitizer, certified Linux x86-64,
  production-CVD/service, materialized-large-file, Sonic1, resource, and
  final matcher/release qualification remain open.
...
- Retain `vba_readn_full()`, the sized `cli_readn` regressions
  `test_ppt_compressed_stream_read_status_is_fail_visible` and
  `test_ppt_compressed_stream_refill_read_status_is_fail_visible`, their
  source guards, and the `ppt-compressed-stream-read-status` manifest
  evidence. Add
...
- Retain the current-source GCC production-linked results: `rust_onenote` 2/2,
  `onenote` 2/2, and `rust_map` 2/2 with zero failures and errors. The three
...
- Treat the Rust 1.97.1 module-filter rerun as compile-only evidence: the crate
  compiled, but its standalone test binary cannot link without production C
  engine symbols such as `cli_checktimelimit` and
  `cli_mark_scan_incomplete`; no new filtered-module pass is claimed.
- Keep complete OneNote corpus, full-C ABI parity, sanitizer, certified Linux
  x86-64, production-CVD/service, materialized-large-file/resource, Sonic1,
  and final parser/release qualification open.
...
- Record the current-source production-linked GCC results: `sis` 1/1 against
  materialized `clam.sis`, `sis_structure` 3/3, `sis_member` 1/1, and
  `sis_map` 3/3 with zero failures and errors. The corpus reaches the exact
  nested MZP matcher after compressed-member extraction.
- Keep this focused evidence separate from complete SIS qualification. Complete

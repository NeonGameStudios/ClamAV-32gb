...
int PASCAL RARReadHeaderEx(HANDLE hArcData,struct RARHeaderDataEx *D)
{
  ...
  try
  {
    if ((Data->HeaderSize=(int)Data->Arc.SearchBlock(HEAD_FILE))<=0)
    {
      ...
      /* RARReadHeaderEx historically reported a short final header as a
       * normal end-of-archive when UnRAR reached EOF before consuming a
       * complete block header.  Preserve the valid old-RAR case where the
       * archive simply has no end block, but reject an EOF reached after
       * unread bytes remain.  The caller must not scan such an archive as a
       * clean, fully inspected container. */
      if (Data->Arc.GetHeaderType() != HEAD_ENDARC &&
          Data->Arc.CurBlockPos != Data->Arc.FileLength())
        return ERAR_BAD_DATA;

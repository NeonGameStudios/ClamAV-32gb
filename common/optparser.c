...
static bool parse_max_scantime(const char *arg, long long *value)
{
    ...
    /* MaxScanTime is stored in the public engine as uint32_t.  Parse the
     * complete decimal token here instead of narrowing through atoi() (or
     * accepting strtoull's optional sign/leading whitespace).  Config-file
     * comments have already been removed by the caller; a direct option
     * insertion must obey the same full-width contract. */
    for (cursor = (const unsigned char *)arg; *cursor; cursor++) {
        uint64_t digit;
...
const struct clam_option __clam_options[] = {
    ...
    {NULL, "report-json", 0, CLOPT_TYPE_STRING, NULL, 0, NULL, FLAG_REQUIRED, OPT_CLAMSCAN, "Write one structured scan report JSON object per input file.", "FILE"},
    ...
    {"MaxTemporarySize", "max-temporary-size", 0, CLOPT_TYPE_SIZE64, MATCH_SIZE, CLI_DEFAULT_MAX_TEMPORARY_SIZE, NULL, 0, OPT_CLAMD | OPT_MILTER | OPT_CLAMSCAN, "This option sets the maximum temporary bytes reserved by one scan, including\nstaged stream input and parser spools. The value of 0 selects the certified 64G\ndefault. Values above 64G are rejected.", "64G"},
    ...
    {"StreamMaxLength", NULL, 0, CLOPT_TYPE_SIZE64, MATCH_SIZE, -1, NULL, 0, OPT_MILTER | OPT_DEPRECATED, "", ""},
    ...
    while (1) {
        ...
        switch (optentry->argtype) {
            ...
            case CLOPT_TYPE_NUMBER:
                if (arg) {
                    if (optentry->name && strcmp(optentry->name, "MaxScanTime") == 0) {
                        if (!parse_max_scantime(arg, &numarg)) {
                            fprintf(stderr, "ERROR: MaxScanTime must be an integer from 0 through %u\n", UINT32_MAX);
                            err = 1;
                            break;
                        }
                    } else {
            ...
            case CLOPT_TYPE_SIZE64:
                ...
                if (!strcmp(optentry->name, "MaxFileSize") || !strcmp(optentry->name, "StreamMaxLength")) {
                    if (lnumarg == 0) {
                         ...
                         * than creating an unbounded scan policy. */
                        lnumarg = CLI_MAX_LARGE_FILESIZE;
                    } else if ((uint64_t)lnumarg > CLI_MAX_LARGE_FILESIZE) {
                        fprintf(stderr, "ERROR: %s cannot exceed 32G in this build\n", optentry->name);
                        err = 1;
                    }
                }
    ...
    while (1) {
        ...
        switch (optentry->argtype) {
            ...
            case CLOPT_TYPE_SIZE64:
                ...
                numarg = lnumarg ? lnumarg : ((optentry->argtype == CLOPT_TYPE_SIZE64) ? lnumlimit : UINT_MAX);
                break;

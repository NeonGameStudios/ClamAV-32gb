...
const struct clam_option __clam_options[] = {
    ...
    {"PCREMaxFileSize", "pcre-max-filesize", 0, CLOPT_TYPE_SIZE64, MATCH_SIZE, CLI_DEFAULT_PCRE_MAX_FILESIZE, NULL, 0, OPT_CLAMD | OPT_CLAMSCAN, "This option sets the maximum filesize for which PCRE subsigs will be executed.\nPCRE requires one contiguous subject, so this build also enforces its 1 GiB\nsingle-allocation ceiling. Values of zero or above 1 GiB select that ceiling;\nthey do not permit an unbounded whole-file mapping. A required PCRE pass that\nexceeds the effective limit makes the scan incomplete and returns non-clean.\nNegative values are not allowed.", "100M"},
            ...
            case CLOPT_TYPE_SIZE64:
                ...
                if (!strcmp(optentry->name, "MaxFileSize")) {
                    if (lnumarg == 0) {
                        ...
                        fprintf(stderr, "ERROR: MaxFileSize cannot exceed 32G in this build\n");
                        err = 1;
                    }
                }

...
mod tests {
    ...
    fn dyntree_works() {
        ...
        let mut rng = TestRng(0x4c48_412d_4c5a_4821);
        // Spam the tree with deterministic pseudo-random values.
        let mut tree = DynHuffTree::new();
        for _ in 0..1_000_000 {
            tree.increment_for_value(rng.sample_below(NUM_LEAVES) as u16);
        }
        validate_tree(&tree);
        println!("--------------\n{}", tree);

        // Spam it with a deterministic, non-uniform distribution.
        let mut cumulative_weights = [0u64; NUM_LEAVES];
        let mut total_weight = 0u64;
        for cumulative in &mut cumulative_weights {
            let weight = (rng.next_u64() as u16) as u64;
            total_weight += weight * weight + 1;
            *cumulative = total_weight;
        }
        for _ in 0..1_000_000 {
            let target = rng.next_u64() % total_weight;
            let sample = cumulative_weights.partition_point(|&upper| upper <= target);
            tree.increment_for_value(sample as u16);
        }
        validate_tree(&tree);
        println!("--------------\n{}", tree);

        // Now use the same deterministic generator as a bit stream.
        let mut rndstream = BitStream::new(&mut rng);
        for _ in 0..1_000_000 {

...
impl<R: Read> Default for LhaDecodeReader<R> {
    fn default() -> Self {
        LhaDecodeReader {
            ...
            output_length: 0,
            decoder: None,
            header_allocation_limit: usize::MAX,
        }
    }
}
...
impl<R: Read> LhaDecodeReader<R> where R::Error: fmt::Debug {
    ...
    pub fn new(rd: R) -> Result<LhaDecodeReader<R>, LhaDecodeError<R>> {
        Self::new_with_allocation_limit(rd, usize::MAX)
    }

    /// Return a new decoder while limiting cumulative allocations made for
    /// each parsed LHA header.
    ///
    /// The same `header_allocation_limit` is applied to the first header and
    /// every header subsequently parsed by [`LhaDecodeReader::next_file`].
    pub fn new_with_allocation_limit(
        mut rd: R,
        header_allocation_limit: usize,
    ) -> Result<LhaDecodeReader<R>, LhaDecodeError<R>> {
        let header = match LhaHeader::read_with_allocation_limit(
            rd.by_ref(),
    ...
    pub fn begin_new(&mut self, mut rd: R) -> Result<bool, LhaDecodeError<R>> {
        let res = match LhaHeader::read_with_allocation_limit(
            rd.by_ref(),
            self.header_allocation_limit,
        ) {
            Ok(Some(header)) => {
                let decoder = DecoderAny::new_from_header(&header, rd);

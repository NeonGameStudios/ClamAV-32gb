...
fn execute_bindgen() -> Result<(), &'static str> {
    ...
    let mut builder = builder()
        ...
        // Keep bindgen's size, alignment, and field-offset assertions enabled.
        // The large-file fork intentionally supports only 64-bit targets, and
        // the checked-in bindings must fail loudly if the C ABI changes.
        .layout_tests(true)
            ...
            llvm_libs
                ...
                .for_each(|filepath_str| match parse_lib_path(filepath_str) {
                    Ok(parsed_path) => {
                        ...
                        println!("cargo:rustc-link-lib={}", parsed_path.link_directive());
                    }
                    Err(_) => {
...
impl ParsedLibraryPath {
    fn link_directive(&self) -> String {
        if self.static_archive {
            format!("static={}", self.libname)
        } else {
            self.libname.clone()
        }
    }
...
fn parse_lib_path(path: &str) -> Result<ParsedLibraryPath, &'static str> {
    ...
    let static_archive = file_name.ends_with(".a") || file_name.ends_with(".lib");

...
pub(crate) fn parse_ink_data(
    ...
    reserve_collection(&mut strokes, ink_data.strokes.len())?;
    for ink_stroke_id in ink_data.strokes.iter().copied() {
        strokes.push(parse_ink_stroke(ink_stroke_id, space, scale_x, scale_y)?);
    }
...
fn parse_ink_path(
    ...
    if props.dimensions.is_empty() {
        return Err(ErrorKind::MalformedOneNoteData(
            "ink stroke properties has no dimensions".into(),
        )
        .into());
    }

    if data.len() % props.dimensions.len() != 0 {
        return Err(ErrorKind::MalformedOneNoteData(
            "ink path length is not divisible by its dimensions".into(),
        )
        .into());
    }
    ...
    reserve_collection(&mut path, dimension_offset)?;
    for (x, y) in x.iter().copied().zip(y.iter().copied()) {
        path.push(InkPoint {
            x: scale_x * x as f32,
            y: scale_y * y as f32,
        });

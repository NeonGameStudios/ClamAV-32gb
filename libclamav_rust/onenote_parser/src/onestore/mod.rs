...
use crate::reader::{reserve_collection_map, reserve_collection_set};
...
pub(crate) fn parse_store(package: &OneStorePackaging) -> Result<OneStore> {
    ...
    reserve_collection_set(&mut parsed_object_spaces, 1)?;
    ...
    for mapping in storage_index.cell_mappings.values() {
        ...
        reserve_collection_map(&mut object_spaces, 1)?;

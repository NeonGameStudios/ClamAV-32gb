...
{
    uint64_t next_size;
    enum clamfi_quota_result quota_result;
    ...
    /* Never submit a truncated prefix to clamd as though it were the whole
     * message. Exact-limit messages are allowed; the first byte beyond the
     * configured limit takes the configured failure action. */
    quota_result = clamfi_quota_add(cf->totsz, maxfilesize, len, &next_size);
...
sfsistat clamfi_eom(SMFICTX *ctx)
{
    ...
    if (cf->over_limit) {
        logg(LOGG_ERROR, "Refusing clean verdict for message exceeding MaxFileSize\n");
        nullify(ctx, cf, CF_BOTH);
        free(cf);
        return FailAction;
    }
